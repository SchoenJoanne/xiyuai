...
公开仓只随附 `xiyu2/` 原创 CC0 集；其余表情包不随仓库分发（版权原因）。要扩充：把你**有合法授权**的图片放进本目录，按上面的 `manifest.json` 结构登记 emotion 标签即可。
...
The public repo ships only the CC0 set under `xiyu2/`; other packs are not distributed. Drop images **you have the rights to** into this directory and register their emotion tags in `manifest.json`.
...
**Sticker compliance is your responsibility.** Evaluate before adding:
