...
import { loadAdminCredentials, ensureSetupToken } from './src/admin.mjs';
import { countAllAccounts } from './src/db.mjs';
...
async function bootstrap() {
  ...
  // Docker/远程首次初始化令牌（local + 零账号时生成并打印；本机 localhost 建号无需它）
  try { ensureSetupToken(countAllAccounts()); } catch (e) { log('warn', `[Setup] token 初始化跳过: ${e.message}`); }
