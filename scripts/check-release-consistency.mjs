...
// ── 2/3) README 版本历史含当前 major.minor（README.en.md 可选·单语发布跳过）────────
const READMES = existsSync(join(ROOT, 'README.en.md')) ? ['README.md', 'README.en.md'] : ['README.md'];
...
// ── 5) 中英功能表行数一致（单语发布：无 README.en.md 则跳过）──────────────────────
if (!existsSync(join(ROOT, 'README.en.md'))) {
  good('单语发布（无 README.en.md）→ 跳过中英功能表行数对账');
} else {
  // 锚点宽容：兼容编号（## 3. …）与两代标题（它能做什么/功能地图 · What it does/Feature map）——dev 旧版与 1.0 发布版 README 双过
  const zh = (read('README.md').split(/^## (?:\d+\.\s*)?(?:它能做什么|功能地图)/m)[1] || '').split('\n---')[0].split('\n').filter(l => /^\|/.test(l)).length;
  const en = (read('README.en.md').split(/^## (?:\d+\.\s*)?(?:What it does|Feature map)/im)[1] || '').split('\n---')[0].split('\n').filter(l => /^\|/.test(l)).length;
