...
ok(buildCrisisReply().includes('12356'), '#5 危机回复含求助热线（完全接管，不走 LLM）');
{
...
const botSrc = readFileSync(join(_dir, '../src/bot.mjs'), 'utf8');
{
  ...
  ok(botSrc.includes("buildCrisisReply(classifyCrisisSeverity"), '源码: crisis high 完全接管路径在位（v1 传严重度分级）');
