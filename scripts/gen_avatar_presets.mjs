...
// 维度
const AGES = [
  { key: 'teen', en: 'young woman with a fresh bright casual look' },   // 🔴 2026-07-03 HOLE 修:去未成年化→成年基线·无校园/校服/低龄词
...
const VIBES = [
  ...
  { key: 'tsundere',  en: 'slightly proud arched brow with hint of shyness, side glance', clothing: 'classy dress with bow or an elegant blouse with twin-tails' },   // 🔴 2026-07-03 HOLE 修:去校服（未成年关联词）
...
function buildPrompt(combo) {
  ...
  // 🔴 2026-07-03 HOLE 修:去校园——去校园关联背景词·统一柔和 pastel 背景(age.school 机制随 teen 去未成年化退役)。
  const background = 'soft pink and pastel gradient background';
  return `Anime portrait of ${age.en}, ${hair.en}, expressive ${vibe.key === 'cool' ? 'sharp' : 'warm'} eyes, ${vibe.en}, wearing ${vibe.clothing}, half-body portrait facing forward, ${background}, ${style}, highly detailed face, no text, no signature, no realistic human, anime illustration only`;
}
