 ...
 *  ① 某存量案例 露营那晚序列：remind/ask_plan 第 2 次起被冷却 ② "突然好想你呀" 短时第 2 条被拦
...
// ── ① 某存量案例 露营序列：第 2 次 remind 被冷却 ──
const now = Date.parse('2026-06-13T15:00:00Z');   // 沪 23:00
const 某存量案例events = [
  ev('remind', '露营', 1.5, now),                 // 21:34 第一次"带齐东西"
  ev('miss_you', '', 1.0, now),                   // 21:54 "突然好想你呀"
];
const coolRemind = isIntentCooled({ intent: 'remind', topic: '露营', events: 某存量案例events, nowMs: now, ackStreak: 1 });
ok(coolRemind.cooled, '① 露营 remind 6h 内第 2 次 → 冷却(止血)');
const coolMiss = isIntentCooled({ intent: 'miss_you', topic: '', events: 某存量案例events, nowMs: now });
...
// ── hint / micro-ack ──
ok(buildIntentDedupHint(某存量案例events, now).includes('remind'), 'buildIntentDedupHint 列出近期 intent');
