...
// ── 静态断言：时序修复（B2·⑤a：时序守卫从常驻移到 PHOTO_COND 索图轮·常驻不授权罐头"刚才没拍好"）──
const comp = readFileSync(new URL('../src/companion.mjs', import.meta.url), 'utf8');
const { PHOTO_COND } = await import('../src/prompt_cond_blocks.mjs');
ok(PHOTO_COND.includes('绝不在本轮第一次提到拍照就说') && !comp.includes('可以自然带过：「刚才没拍好」'),
   '时序守卫(首轮不说"刚才没拍好")在 PHOTO_COND(B2 从常驻移索图轮)·常驻不授权罐头');
