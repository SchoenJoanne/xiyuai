...
dotenv.config({ path: process.env.ENV_FILE || '.env' });   // 发布工序②：env 驱动（原硬编码生产路径已泛化）
...
const REF_PATH = process.env.REF_PATH || '';   // 发布工序②：参考图路径 env 驱动（空=跳过 i2i·原硬编码生产路径已泛化）
...
const refBuf = REF_PATH ? readFileSync(REF_PATH) : null;   // 无 REF_PATH → 纯文字组仍可跑
