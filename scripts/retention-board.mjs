 ...
 *
 * 用法：DB_PATH=./data/bot.db npm run retention:board
...
// ── 24h 主动窗口维度（PR：照亮超窗口流失·proactive 够不够得着）────────────────
// 微信/iLink：用户最后真实发言起 ~24h 内 bot 才能主动 sendMessage（context_token 窗口）；
// 超 24h 窗口关、proactive（含早安续命器）被平台禁言发不出——这批「超窗口流失」完全不进
// turns 表（生成前预检跳过 / 发送 ret=-2 都不落库）→ 原看板量不到 = 北极星仪表盲区。
// 本维度按「距最后真实发言时长」补出窗口状态，把 proactive 够不着的黑洞照出来。
// 纯时间戳判定，不碰对话内容。SQL 等价：(now - max(user.created_at)) ⪋ 24h。
export const WINDOW_HOURS = 24;
export function windowStatus(lastRealUserMs, nowMs, windowH = WINDOW_HOURS) {
  if (lastRealUserMs == null) return { state: 'never', hoursSince: null };
  const hoursSince = (nowMs - lastRealUserMs) / 3600e3;
  return { state: hoursSince <= windowH ? 'open' : 'closed', hoursSince };
}
// 距最后发言时长分布桶（24h 线两侧各分档：看多少人卡在鸿沟附近 / 超窗口多久）。
export const WINDOW_BUCKETS = ['≤6h', '6-12h', '12-18h', '18-24h', '24-48h', '48-72h', '>72h'];
export function windowBucket(hoursSince) {
  if (hoursSince == null) return null;
  const h = hoursSince;
  if (h <= 6)  return '≤6h';
  if (h <= 12) return '6-12h';
  if (h <= 18) return '12-18h';
  if (h <= 24) return '18-24h';
  if (h <= 48) return '24-48h';
  if (h <= 72) return '48-72h';
  return '>72h';
}
// 聚合：一组用户的「最后真实发言 ms」→ 窗口开/关/从没聊过 计数 + 时长分布桶。
export function windowAggregate(lastUserMsList, nowMs, windowH = WINDOW_HOURS) {
  const buckets = Object.fromEntries(WINDOW_BUCKETS.map((b) => [b, 0]));
  let open = 0, closed = 0, never = 0;
  for (const lastMs of lastUserMsList) {
    const w = windowStatus(lastMs, nowMs, windowH);
    if (w.state === 'never') { never++; continue; }
    if (w.state === 'open') open++; else closed++;
    buckets[windowBucket(w.hoursSince)]++;
  }
  return { total: lastUserMsList.length, open, closed, never, buckets, windowH };
}
...
function main() {
  ...
  for (const [cid, turns] of byComp) {
    ...
    users.push({
      ...
      lastUserMs: Math.max(...realUserMs),     // 最后一次真实发言（24h 窗口判定基准·纯时间戳）
  ...
  // ── 24h 主动窗口维度（proactive 够不够得着·照亮超窗口流失）──
  const nowMs = Date.now();
  const wagg = windowAggregate(users.map((u) => u.lastUserMs), nowMs);
  console.log(`\n── 24h 主动窗口维度（proactive 够不够得着·照亮超窗口流失）──`);
  console.log(`  窗口判定：距最后真实发言 ≤24h=开(proactive 发得出) / >24h=关(平台禁言·主动消息发不出)`);
  console.log(`  窗口开（续命器够得着）.......... ${wagg.open}  (${pct(wagg.open, N)})  ${list(u => windowStatus(u.lastUserMs, nowMs).state === 'open')}`);
  console.log(`  窗口关（proactive 失效·超窗口）.. ${wagg.closed}  (${pct(wagg.closed, N)})  ${list(u => windowStatus(u.lastUserMs, nowMs).state === 'closed')}`);
  console.log(`  ★ 续命器天花板：${wagg.closed}/${N} 个真实用户此刻窗口已关·proactive(含早安)够不着·只能靠内在动力自回`);
  console.log(`  距最后发言时长分布（看多少人卡在 24h 鸿沟附近 / 超窗口多久）：`);
  for (const b of WINDOW_BUCKETS) {
    if (b === '24-48h') console.log(`   ──── 24h 窗口线（以下窗口已关·proactive 发不出）────`);
    const tag = b === '18-24h' ? '   ← 窗口将关·临门一脚区(21-23.5h lastcall 在救)'
              : b === '>72h'   ? '   ← 超窗口越久·大概率永久流失'
              : '';
    console.log(`    ${b.padEnd(6)}: ${wagg.buckets[b]}${tag}`);
  }
  console.log(`  per-user 窗口状态（距最后真实发言·纯时间戳）：`);
  const wlist = users.map((u) => ({ uid: u.uid, name: u.name, w: windowStatus(u.lastUserMs, nowMs) }))
    .sort((a, c) => (a.w.hoursSince ?? Infinity) - (c.w.hoursSince ?? Infinity));
  for (const s of wlist) {
    const h = s.w.hoursSince;
    const hStr = h == null ? '从没聊过' : h < 24 ? `${h.toFixed(1)}h` : `${(h / 24).toFixed(1)}d`;
    const lab = s.w.state === 'open' ? '窗口开' : s.w.state === 'closed' ? '窗口关·发不出' : '从没聊过';
    console.log(`    u${s.uid} ${s.name}  距最后发言 ${hStr}  [${lab}]`);
  }
  ...
  const farClosed = users.filter(u => { const w = windowStatus(u.lastUserMs, nowMs); return w.state === 'closed' && w.hoursSince > 72; }).length;
  console.log(`  超窗口流失（proactive 救不了）：${wagg.closed}/${N} 人此刻窗口已关(>24h 未发言·主动消息发不出)，其中 >72h ${farClosed} 人=大概率永久流失`);
  console.log(`  ★ 两类要分开治：窗口内「没接住」${P.zero24h} 条 proactive 24h 零回复(内容/时机·PR-2 在治) ≠ 超窗口「发不出」${wagg.closed} 人(平台天花板·proactive 无能为力·只能靠内在动力)`);
  console.log(`\n════ 看板完（纯只读，无任何回写）════`);
}
