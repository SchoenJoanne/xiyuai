...
import {
  ...
  windowStatus, windowBucket, windowAggregate, WINDOW_HOURS,
...
// ── 24h 主动窗口维度：窗口状态 / 桶 / 聚合（照亮超窗口流失）──
const NOW = ms('2026-06-15', 12);
ok(WINDOW_HOURS === 24, 'WINDOW_HOURS=24');
ok(windowStatus(NOW - 23 * 3600e3, NOW).state === 'open', '距发言 23h → 窗口开(proactive 发得出)');
ok(windowStatus(NOW - 24 * 3600e3, NOW).state === 'open', '恰 24h 边界 → 仍开(≤24h)');
ok(windowStatus(NOW - (24 * 3600 + 60) * 1e3, NOW).state === 'closed', '24h01m → 窗口关(边界外·发不出)');
ok(windowStatus(NOW - 25 * 3600e3, NOW).state === 'closed', '距发言 25h → 窗口关');
ok(windowStatus(null, NOW).state === 'never' && windowStatus(null, NOW).hoursSince === null, 'null=从没聊过 → never');
// 时长桶分档（24h 线两侧）
ok(windowBucket(5) === '≤6h' && windowBucket(12) === '6-12h' && windowBucket(20) === '18-24h', '桶：窗口内分档');
ok(windowBucket(30) === '24-48h' && windowBucket(60) === '48-72h' && windowBucket(100) === '>72h', '桶：超窗口分档');
ok(windowBucket(null) === null, 'never 无桶');
// 聚合：开3(2h/12h/23h) + 关2(30h/100h) + never1
const wa = windowAggregate([NOW - 2 * 3600e3, NOW - 12 * 3600e3, NOW - 23 * 3600e3, NOW - 30 * 3600e3, NOW - 100 * 3600e3, null], NOW);
ok(wa.total === 6 && wa.open === 3 && wa.closed === 2 && wa.never === 1, '聚合：total6 开3 关2 never1');
ok(wa.buckets['≤6h'] === 1 && wa.buckets['18-24h'] === 1 && wa.buckets['24-48h'] === 1 && wa.buckets['>72h'] === 1, '聚合桶：各档计数对');
