 ...
 *       DB_PATH=./data/bot.db node scripts/retention_dashboard.mjs
 *       EXCLUDE=acc1,acc2 node scripts/retention_dashboard.mjs   # 排除测试号(逗号分隔·默认空)
 *
 * 🔴 红线：纯只读分析工具·零改动生产行为·绝不把留存指标接进她的行为（见 README 反操纵红线）。
...
console.log(`\n提示：今日活跃/关系分布/活跃3天+ 是运维健康观察项（🔴只读·绝不接入她的行为），隔几天再跑对比。\n`);
db.close();
