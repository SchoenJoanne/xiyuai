...
const SRC_LINE_EXEMPT = [
  ...
  '输出绝不出现：用户',                              // PR-3.2 commit B self_facts 生成 prompt 的禁令行
  'REL_FORBIDDEN_WORDS',                            // PR-3.2 guardGeneratedText 关系禁词检测器定义
  "blob.includes('用户')",                          // PR-3.2 guardGeneratedText 检测器自身
  // 2026-07-03 ③ SOCIAL_CIRCLE Step1 死文本豁免（🔴 豁免只收死文本·真 prompt 不豁免·见 CLAUDE.md）：
  'REL_GUARD_EXTRA',                                // character_seed.mjs 关系禁词表（guard-def·非 prompt）
  'SOCIAL_USER_LITERAL_RE',                         // social_circle.mjs 该 guard 自己的检测 regex 定义
  '咱俩/用户/对方',                                  // social_circle.mjs prompt 内负向禁令（教 LLM 别说·死文本·同"绝不写'用户'"族）
  // 🔴 SOCIAL_CIRCLE 开闸前置债（闸动债动·真 prompt 临时豁免·非永久死文本）：
  //    闸开前须 social_circle.mjs:196「用户问起你的生活」reword「用户」→「他」（见 docs/GATED_FEATURE_DEBTS.md「SOCIAL_CIRCLE」）。
  '用户问起你的生活',                                // social_circle.mjs:196 真措辞债·SOCIAL_CIRCLE 默认关未上线
];
...
function scanPage(file, src) {
  const leaks = [];
  // 🔴 2026-07-03 ③：先剥多行 HTML 注释（<!-- … --> 跨行·内容置空格保留行号）——旧版只跳
  //    「行首 <!--」，漏掉多行注释的续行（dashboard.html:778-779 误报根因）。结构修 > 逐行豁免。
  ...
  scan.forEach((line, i) => {
    ...
    leaks.push(`${file}:${i + 1}: ${orig[i].trim().slice(0, 80)}`);
  });
  return leaks;
}
