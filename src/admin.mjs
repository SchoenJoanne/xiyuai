...
// 凭据落 XIYU_DATA_DIR（Docker=可写的 /app/data 卷·非 root 亦可写·且随卷持久=重启不轮换）；
// 未设时回退项目根（裸机旧行为字节不变）。修 Docker 非 root 写 /app 根 EACCES。
const DATA_DIR = process.env.XIYU_DATA_DIR || path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const CRED_FILE = path.join(DATA_DIR, '.admin-credentials');
...
/**
 * Docker/远程首次初始化令牌（不弱化安全）。
 * local 模式 + 零账号 + 未显式设 XIYU_SETUP_TOKEN 时，自动生成一次性强随机 setup token 并打印日志：
 *   · 本机 localhost 建号：isLocalhostRequest 直接放行，无需它；
 *   · Docker/远程建号：容器端口映射下 req.ip=网桥网关≠127.0.0.1，用此 token 走同一 setupTokenMatches
 *     校验路径（与显式设 XIYU_SETUP_TOKEN 等价）。零账号窗口一过（首账号建成）即永久失效。
 */
export function ensureSetupToken(userCount = 0) {
  if (process.env.XIYU_SETUP_TOKEN) return;                                  // 用户已显式设，尊重
  const isLocalMode = (process.env.AUTH_MODE || 'local').toLowerCase() !== 'email';
  if (!isLocalMode || userCount > 0) return;                                 // 非 local 或已初始化
  const token = crypto.randomBytes(24).toString('hex');
  process.env.XIYU_SETUP_TOKEN = token;
  log('info', '======================================================');
  log('info', '  [Setup] 首次初始化令牌（Docker/远程首次建号用）：');
  log('info', `  ${token}`);
  log('info', '  · 本机 localhost 打开 setup 页建号：无需填它');
  log('info', '  · Docker/远程：在 setup 页「初始化令牌」栏填入此值');
  log('info', '======================================================');
}
