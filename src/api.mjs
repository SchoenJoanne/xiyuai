...
import { generatePersonaFacts, generateAvatarCandidates, embedText, generateSelfFacts } from './ai.mjs';
...
// PR-3.2 C3：创建链给 self seed 通电。generateSelfFacts 内含输入漏斗 + 安全闸（危险 seed 不进 LLM）+ retry + safe_default，
//   本 fn 只编排 + 持久化。fire-and-forget 不阻塞创建响应（同 asyncGeneratePersonaFacts）。
async function asyncGenerateSelfSeed(companion) {
  try {
    const r = await generateSelfFacts(companion);             // {status, meta, llmCalls}·meta 在 4 status 全非空（含 rejected 带 verdict）
    if (r?.meta) saveCharacterSeedMeta(companion.id, r.meta);  // 唯一授权写入口·rejected 也落库（防无限重试 + 可审计）
    log('info', `[SelfSeed] companion=${companion.id} status=${r?.status} llmCalls=${r?.llmCalls ?? 0}`);
  } catch (e) {
    log('error', `[SelfSeed] async 生成失败 companion=${companion.id}: ${e.message}`);
  }
}
...
import {
  ...
  getCompanionTimeline, savePersonaFacts, getPersonaFacts, hasPersonaFacts, saveCharacterSeedMeta,
  getDailySchedule, shanghaiDateKey, shanghaiYMD,
  matchAvatarPresets, countAvatarPresets, getAvatarPresetByFileName,
...
// 提醒 date 的"今天"走上海日历（自托管 UTC 下避免错日：B2）。
...
function companionSummary(companion) {
  ...
  return {
    id: companion.id,
    name: fallbackText(companion.name, DEFAULT_COMPANION_NAME),
    ...
    proactive_daily_target: Number.isFinite(Number(companion.proactive_daily_target))
      ...
      : 10,
    // v1.22 频率语义档灰度闸：把后端 env 暴露给【静态】dashboard（无模板注入·只能经 API 取 flag）。
    // 🔴 与 proactive.mjs isFreqSemanticOn 同一 env 名同一正则（两处互指·故意不进 reach-guard 文件）。
    // 默认 false=dashboard 渲染旧 0-30 数值滑块=零变更；=1 时渲染 2 档语义（随她/少一些）。
    proactive_freq_semantic: /^(1|true|on|yes)$/i.test(process.env.PROACTIVE_FREQ_SEMANTIC || ''),
...
// POST /api/billing/admin/grant-pro  (运营手动开通，需要 admin 登录)
//   M1: 鉴权统一走 requireAdmin（scrypt 凭据 + admin JWT + 常量时间验签），
//   与其余 /admin/* 端点一致；删除独立的静态 ADMIN_TOKEN 比对（非常量时间且在体系外）。
//   调用方先走 admin 登录拿 token，再以 Authorization: Bearer / x-admin-token 携带。
//   body: { user_id, days }
router.post('/billing/admin/grant-pro', requireAdmin, (req, res) => {
...
router.post('/auth/wechat-bind', requireAuth, (req, res) => {
  ...
  try {
    ...
    if (!companion.character_seed_meta) asyncGenerateSelfSeed(companion);   // PR-3.2 C3：seed 通电（幂等·已建跳过）
...
router.get('/companions/:id/avatar/suggest', requireAuth, async (req, res) => {
  const id = intId(req.params.id); if (!id) return err(res, 'id 无效');
  const c = requireOwnedCompanion(req, res, id); if (!c) return;
  // 🔴 2026-07-03 child-safety（db.mjs:66 HOLE 修·对齐 avatar/generate 端点 isClearlyAdult·消硬化不对称）：
  //   非明确成年不匹配预设（含冻结存量未成年）——冻结的是账号数据、不是低龄图像义务。
  const { isClearlyAdult } = await import('./visual_identity.mjs');
  if (!isClearlyAdult(c.age)) {
    log('warn', `[Avatar] 🔴 拒绝非明确成年预设匹配 companion=${id} age=${c.age}（child-safety·对齐 generate 端点）`);
    return err(res, '该角色未设明确成年年龄，不能匹配头像', 403);
  }
...
router.post('/companions/:id/avatar/select-preset', requireAuth, async (req, res) => {
  const id = intId(req.params.id); if (!id) return err(res, 'id 无效');
  const c = requireOwnedCompanion(req, res, id); if (!c) return;
  // 🔴 2026-07-03 GPT·fail-closed（对齐 generate/suggest·消不对称）：非明确成年不选预设。
  const { isClearlyAdult } = await import('./visual_identity.mjs');
  if (!isClearlyAdult(c.age)) {
    log('warn', `[Avatar] 🔴 拒绝非明确成年选预设 companion=${id} age=${c.age}（child-safety）`);
    return err(res, '该角色未设明确成年年龄，不能选用头像', 403);
  }
  ...
  // 🔴 2026-07-03 GPT·断流通（不可复用为头像源）：必须是【enabled=1】在册 preset——只验磁盘存在会漏
  //   （disabled teen preset 的 webp 物理文件仍在→原 existsSync 能过·绕过 ④ disable）。DB enabled 才是真闸。
  const preset = getAvatarPresetByFileName(fileName);
  if (!preset || !preset.enabled) return err(res, '该预设不可用', 404);
  // 验证文件存在（双保险）
...
router.post('/companions/:id/avatar/generate', requireAuth, async (req, res) => {
  const id = intId(req.params.id); if (!id) return err(res, 'id 无效');
  const c = requireOwnedCompanion(req, res, id); if (!c) return;
  // 🔴 v1.23 child-safety 步①+anime gate（裁决:child-safety 不分画风）：anime 头像也 fail-closed——
  //   非明确成年(age 非 ≥18 有效值·含 null/异常/<18)不生成头像(原 age<18→'teenage school girl' anime=未成年描述)。
  const { isClearlyAdult } = await import('./visual_identity.mjs');
  if (!isClearlyAdult(c.age)) {
    log('warn', `[Avatar] 🔴 拒绝非明确成年 anime 头像生成 companion=${id} age=${c.age}（child-safety·不分画风）`);
    return err(res, '该角色未设明确成年年龄，不能生成头像', 403);
  }
  try {
...
// 🔴 合规硬门槛（《人工智能拟人化互动服务管理暂行办法》2026-07-15 生效·路线A 永久禁止未成年人设新建/改入<18）：
//    最低 companion 年龄 16→18。MIN==ADULT==18 = 极简年龄门（全员成年），原 [MIN,ADULT) 强制 nsfw=0 的"未成年带"自然清空。
//    🔴 这是 route 层友好报错（400）；确定性兜底在 db.createCompanion（新建<18 throw）/ persona INSERT（新导入<18 clamp）。
//    🔴 冻结存量兼容例外：已在库 age<18 的历史遗留（未成年）= 完全冻结保护——其正常编辑（保持<18）放行，
//       此门只挡【新建<18】与【把已存非未成年改成<18】，绝不拦冻结存量的正常迭代更新（否则"后续带着他迭代"做不到）。
const MIN_COMPANION_AGE = 18;
const ADULT_COMPANION_AGE = 18;

/**
 * AI 虚拟角色（companion）年龄合规守门：
 *   - 新建 age < 18（existingCompanion=null）→ 抛错（调用方返回 400）
 *   - 编辑：把已存的"非未成年/无龄"companion 改成 age<18 → 抛错（不许编辑制造新的<18）
 *   - 🔴 编辑：已存 age<18 的冻结存量（未成年）→ 放行（保留<18·正常迭代更新不被拦）
 *   - age >= 18 → 按用户设置（MIN==ADULT 后"未成年带"为空集；<18 存量仍强制 nsfw=0）
 *   - age 未提供（PATCH 场景）→ 不做改动
 */
export function applyCompanionAgeGuard(data, existingCompanion = null) {   // export=坏版本验红 test affordance·零运行时（route 层 400 友好报错）
  const out = { ...data };
  let age = null;
  if (out.age !== undefined && out.age !== null && out.age !== '') {
    ...
    if (age < MIN_COMPANION_AGE) {
      // 🔴 路线A：拒绝【新建<18】与【把已存非未成年 companion 改成<18】；
      //    例外=【已存 age<18 的冻结存量（未成年)】——其 age 本就<18，正常迭代更新（改昵称等·body 仍带 age=16）放行，
      //    判据=existingCompanion 已存在且其当前 age 已<18（既非新建、也非把成年改成未成年）。绝不改其 age。
      const _existAge = existingCompanion && existingCompanion.age != null ? Number(existingCompanion.age) : null;
      const _isFrozenLegacyMinor = _existAge != null && Number.isFinite(_existAge) && _existAge < MIN_COMPANION_AGE;
      if (!_isFrozenLegacyMinor) {
...
router.post('/companions', requireAuth, (req, res) => {
  ...
  try {
    ...
    asyncGenerateSelfSeed(c);   // PR-3.2 C3：新建即给 self seed 通电（异步不阻塞）
...
router.put('/companions/:id/scene', requireAuth, (req, res) => {
  ...
  // PR-3·B：手动直写 current_scene(裸 location) 必清 sidecar meta，避免残留旧 food_state/transition 误导 buildSceneHint
  patchCompanion(id, { current_scene: scene, current_scene_meta: null, scene_history: JSON.stringify(history) });
  ...
  try {
    ...
    (async () => {
      try {
        ...
        if (!ctx?.token) return;
        const sys = `你叫${comp.name || DEFAULT_COMPANION_NAME}。当前场景：他刚刚打电话把你从熟睡中吵醒。
...
router.post('/companions/:id/visual-identity/lock',
  ...
  try {
    const { saveReferenceImage, resetVisualIdentity, candidatePath, isClearlyAdult } = await import('./visual_identity.mjs');
    // 🔴 v1.23 child-safety 步① fail-closed（项3 覆盖：锁脸端点把候选锁为 i2i ref·原无 age 闸=旁路）：
    //   只有明确成年(≥18 有效 age)才能锁人脸 ref。防把 id=16 等的旧 minor 候选(闸前生成)锁成 i2i 输入。
    if (!isClearlyAdult(c.age)) {
      log('warn', `[API] 🔴 拒绝非明确成年锁脸 companion=${id} age=${c.age}（child-safety·未知/异常/未成年不锁人脸 ref）`);
      return err(res, '该角色未设明确成年年龄，不能锁定人脸参考', 403);
    }
...
router.post('/companions/:id/age-attestation', requireAuth, (req, res) => {
  ...
  // 🔴 B2 安全关键：成年校验用上海"今天"的年份算年龄，自托管 UTC 下避免跨年边界误判临界用户。
  const nowYear = shanghaiYMD().year;
...
export function startApiServer() {
  ...
  // Turnstile sitekey 注入：静态页里的 __TURNSTILE_SITEKEY__ 占位在服务时替换为 env 值。
  // 默认用 Cloudflare 官方测试 key（任意域名恒通过）→ 自托管开箱即可注册；
  // 真实防刷请在 .env 设 TURNSTILE_SITEKEY + TURNSTILE_SECRET（两者配对）。sitekey 无敏感性（本就客户端可见）。
  const TURNSTILE_SITEKEY = process.env.TURNSTILE_SITEKEY || '1x00000000000000000000AA';
  for (const page of ['/app/auth.html', '/app/forgot.html']) {
    app.get(page, (_req, res) => {
      try {
        const html = readFileSync(path.join(PUBLIC_DIR, page.slice(1)), 'utf8').replaceAll('__TURNSTILE_SITEKEY__', TURNSTILE_SITEKEY);
        res.type('html').send(html);
      } catch { res.sendFile(path.join(PUBLIC_DIR, page.slice(1))); }
    });
  }
