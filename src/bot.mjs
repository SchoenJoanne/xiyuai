...
import {
  ...
  getConversationContext, saveConversationTurn, recentOutboundStickerActive,
  ...
  listOpenLoops,                                   // topic 主动话头取料：没了结的事/约定
  getTopicOpensToday, bumpTopicOpensToday,         // 🔴 topic 每日预算（日界重置·非时间）
} from './db.mjs';
import { buildSystemPrompt, buildFirstTurnHint, buildFirstTurnFallback } from './companion.mjs';
...
import { buildSocialPromptHint, socialAllowedFor, pickFriendAnchor } from './social_circle.mjs';  // 社交圈 Step1 reply 按需注入 + 保护门控 + 闺蜜名单一源(character_seed 统一)
import { getSocialCircle } from './db.mjs';                   // 社交圈 Step1 档案读
...
import { safeOutboundReply, inboundIsBlocked, detectSafetyRisk, detectCrisisLevel, classifyCrisisSeverity, buildCrisisReply, scrubPersonaLeak, scrubPhotoImpersonation, scrubConflictRedline, scrubFabricatedIllness, scrubPeriodDisclosure, hitsFirstTurnGuiltWord, scrubTopicOpenerRedline } from './moderation.mjs';
import { topicClosureSignal, buildTopicOpenerHint, topicOpenerEnabledFor, TOPIC_OPEN_DAILY } from './topic_opener.mjs';   // 回库 2026-06-25：对话中她主动开新话头（同步 reply 路径·绝不读沉默）
import { buildCanonicalFactSnapshot, analyzeFactGuard, scrubUnsupportedClaimConfirmation, detectFactTopics, scrubCriticalSlotContradiction, scrubUnsupportedSharedTopicConfirmation, scrubSelfIdentityNumberDrift } from './fact_guard.mjs';  // PR-3 persona/共同往事 硬事实守卫 + 批C·C2/G1 年龄漂移出站门
import { seedMetaToFactItems, selectSelfSeedSoftItems } from './character_seed.mjs';   // PR-3.2 C2 召回桥 + C3 软常驻子集（在此调·避环·fact_guard 不反向 import character_seed）
import { buildSceneHint, scrubSceneJump } from './scene_state.mjs';  // PR-3·B scene 结构化守卫（B-min sidecar）
...
import { clampReachForReply } from './proactive_policy.mjs';  // Step0: reply 路径补 reach-guard(独立闸 REPLY_REACH_GUARD·钳缺席质问)
...
import { PHOTO_COND } from './prompt_cond_blocks.mjs';   // B2·⑤a：索图轮条件块单一权威源
...
import { maybeSleepBlock, getOrRefreshTodaySchedule, isSleepingNow, drainMissed, isSleepGuardOn } from './sleep.mjs';
import { touchUsageSession, computeUsageNotice, markUsageNoticed } from './anti_addiction.mjs';   // 防沉迷 2h 提示(法规②·停板A)
import { getBodyEventsToday } from './db.mjs';   // P2→P1 即时状态锚定 presence
...
export function enqueueOrRunTurn(turn) {
  ...
  // v1.23 SLEEP_GUARD：wakeHint 在 burst 里粘住——叫醒发生在首条，后续条已醒(无 wakeHint)不该把它冲掉。
  // 🔴 BUG#1 修：missed 承接(drainMissed 命中)只在 burst 首条·后续条 drain 返0→wakeHint 基底 truthy 但无承接，
  //    原 `!turn.wakeHint` 不触发→富版被无承接版冲掉=她对睡眠期消息装没看见。改为：新条没 wakeHint，
  //    或旧条带承接而新条没带 → 保留旧的富版(连同 wakeHintHasMissed 标记)。
  const _prevTurn = b.turn;
  if (_prevTurn && _prevTurn !== turn && _prevTurn.wakeHint
      && (!turn.wakeHint || (_prevTurn.wakeHintHasMissed && !turn.wakeHintHasMissed))) {
    turn.wakeHint = _prevTurn.wakeHint;
    turn.wakeHintHasMissed = _prevTurn.wakeHintHasMissed;
  }
...
export async function handleMessage(rawMsg, botContext = {}) {
  ...
  try {
    ...
    // v1.23 SLEEP_GUARD：C 临醒被叫醒(reason=woken_by_user)直接回·迷糊档；missed 解耦(ii)醒后首条排空+承接。
    let wakeHint = '';
    let wakeHintHasMissed = false;   // 🔴 BUG#1:本条 wakeHint 是否含 missed 承接(drainMissed 命中)·供 burst 粘住保留富版
    try {
      ...
      // C 叫醒 + missed 解耦(ii)：醒着/刚被叫醒时排空睡眠期攒的 missed，prompt 自然带"刚看到你发的"。
      // drainMissed 幂等（drain 后置 consumed=1，下条返 []）；闸关→整段不跑=零变更。
      if (isSleepGuardOn()) {
        const _drained = drainMissed(companion.id);
        wakeHintHasMissed = _drained.length > 0;   // 🔴 BUG#1:承接只在 burst 首条 drain 命中时出现(后续条 drain 返0)
        if (sleepGate.reason === 'woken_by_user') {
          wakeHint = `\n【★ 刚被他叫醒】你本来在睡（快到平时起床点了），是他发消息把你叫醒的——迷糊、带点起床气和睡意、声音软，别像没睡过一样精神满满（"唔…被你吵醒啦""才几点呀…困死了"）；但因为是他叫的不会真生气，反而有点黏。${_drained.length ? `他在你睡着时还发过 ${_drained.length} 条，自然回一句"我刚醒，看到你发的了"，别逐条复读。` : ''}`;
          log('info', `[Bot] 临醒被叫醒回复 companion=${companion.id} missed=${_drained.length}`);
        } else if (_drained.length) {
          wakeHint = `\n【★ 你刚睡醒/回神，看到他在你没空时发过 ${_drained.length} 条消息】自然回应一句"我刚看到你发的了"，别假装没看见、也别逐条复读。`;
        }
      }
    } catch (e) {
    ...
    enqueueOrRunTurn({ companion, binding, ctx, botId, fromUser: msg.fromUser, contextToken: msg.contextToken, userText, wakeHint, wakeHintHasMissed });
    return;
  } catch (err) {
...
async function processUserTurn({ companion, binding, ctx, botId, fromUser, contextToken, userText, userParts = null, wakeHint = '', _mergeDepth = 0 }) {
  ...
  try {
    ...
    if (photoIntent.type === 'strong_photo_request' && photoEsc.level < 2) {
      // 🔴 防沉迷用量测量 co-located：照片旁路轮也 advance last_user_reply_at，须同步保活会话锚点
      // （否则重度索图用户时长永久归零·停板A H1）。必须在 recordUserReplied 之前调（读到 prev）。fail-open。
      try { touchUsageSession(companion); } catch {}
    ...
    // 贴纸节流 v1.27（reply 路径）：
    //   base=开关+有素材；D2 冷却=最近 4 条发出消息内已有贴纸→本轮不发（轮次口径·零时钟）。
    //   allowStickers=base && !cooling：既锁掉「看不到就说不出」（hint 不注入），也在出口(1235)硬 drop
    //   LLM 无提示仍吐的标记（缺口①：开关关/冷却时的幻觉标记）。
    const stickerBase = !!companion.sticker_reply_enabled && hasStickers();
    const stickerCooling = stickerBase && recentOutboundStickerActive(msg.fromUser, botId, 4);
    // 质/情境门控（缺口②·v1.28·dark-then-enable·默认 OFF 字节等价）：吵架(冲突态)/危机轮不注入贴纸。
    // 折进 allowStickers → hint 软层(下方)+输出硬层(parseStickerMarkers)双层一次到位·与量正交·判定=纯函数单一来源。
    const allowStickers = stickerBase && !stickerCooling
      && !stickerContextBlocked({ crisisLevel: _crisisLevel, arcState: arcCtx.arcState, escLevel: esc.level });
    ...
    // 🔴 Step0(2026-06-28): reply 路径补 reach-guard——此前直送未钳的 missing/neglect 给 buildEmotionPromptHint，
    // 而 reply 是最高频面，缺席质问(你是不是把我忘了/你怎么才来/还以为你不来了)在每条回复裸奔(reach-guard 原本
    // 只挡 proactive)。独立闸 REPLY_REACH_GUARD(与 proactive 解耦·独立 dogfood/回滚)钳 missing≤2 / uneasy→none
    // ——只钳喂质问的 emotionHint；reunionHint(上面用 raw neglectStage·久别温暖/和好)不动；闸关=字节一致旧行为。
    const _reach = clampReachForReply({ missingLevel, neglectStage: reunionHint ? 'none' : neglectStage });
    const emotionHint = Number(companion.safe_mode) ? '' : buildEmotionPromptHint(emotionState, { missingLevel: _reach.missingLevel, neglectStage: _reach.neglectStage, dailySchedule, arcActive: arcCtx.active, bodyLowEnergy: isPeriodHeavyWindow(periodCtx), pmsActive: isPmsActive(periodCtx) });
    ...
    // 社交圈 Step1：🔴按需注入(buildSocialPromptHint 内部 isSocialCircleOn+isSocialQueryIntent 双闸)+🔴保护门控
    // socialAllowedFor(age<18/未知 + safe_mode 全挡·child-safety·含 冻结存量未成年)·独立通道(不并 emotionHint)·fail-open。
    let socialHint = '';
    try { if (socialAllowedFor(companion)) socialHint = buildSocialPromptHint(getSocialCircle(companion.id), userText); }
    catch (e) { log('warn', `[SocialCircle] 注入构建失败 companion=${companion.id}: ${e.message}`); }
    ...
    // PR-3.2 C3：自有事实软常驻——只 reply path 显式开（includeSelfSeedSoftLayer:true）并传子集；
    //   默认安全：proactive/playground 不传 → 默认 false → 吃不到软段（不漏）。null/坏 meta → [] → 软段静默。
    // 🔴 闺蜜名单一事实源（SOCIAL_CIRCLE 前置·统一 character_seed 与 social_circle 各自 id-seeded 起名的穿帮）
    const _friendNick = pickFriendAnchor(companion).nickname;
    const _selfSeedSoftItems = selectSelfSeedSoftItems(companion.character_seed_meta, { friendNick: _friendNick });
    // P2→P1（2026-06-24·停板B）即时状态锚定 presence（reply 路径·fail-open）。
    const _presence = (() => {
      try {
        const slp = getOrRefreshTodaySchedule(companion.id);
        const dayContext = getDayContext(companion, todayKey);   // 学期/寒暑假/法定节假日（同喂日程生成）
        const shNow = new Date(Date.now() + 8 * 3600_000);
        const nowMin = shNow.getUTCHours() * 60 + shNow.getUTCMinutes();
        const expectedBand = expectedActivityBand(companion, dayContext.dayKind, nowMin);  // 此刻按身份该在做什么
        return { wokeAt: slp?.today_wake_at || 0, isSleeping: isSleepingNow(companion.id),
          lastProactiveSentAt: (Number(companion.last_proactive_sent_at) || 0) * 1000, bodyEventsToday: getBodyEventsToday(companion.id),
          dayContext, expectedBand };
      } catch { return null; }
    })();
    let systemPrompt = buildSystemPrompt(companion, { memories, userProfile, recentTurns, longTermDigest, promptMode: 'reply', dailySchedule, recentSchedules, personaFacts, preferences, shapingHint, worksHint, socialHint, openLoopsHint, includeSelfSeedSoftLayer: true, selfSeedItems: _selfSeedSoftItems, presence: _presence }) + stickerHint + emotionHint + reunionHint + shapingConfirmHint + (arcCtx.active ? '' : escalationDirective(esc.level)) + (arcCtx.directive || '') + (wakeHint || '');
    ...
    // PR-3 硬事实守卫（prompt 主防）：canonical snapshot + 意图判定 + 共同往事守卫，仅涉及事实/身份/往事
    // 意图时注入（普通闲聊零注入·不刻板）；unsupportedSpans 传到出站做确认式复述兜底（闭环 gate）。fail-open。
    let _factGuardSpans = [];
    let _factTopics = null, _factSnapshot = null;   // commit B：提升供出站闭域 gate 复用
    try {
      _factTopics = detectFactTopics(userText);   // PR-3.1·A topic-aware：用户本轮碰了哪些 fact slot
      // PR-3.2 C2：AI 自有 backstory 召回——companion.character_seed_meta(getCompanionById=SELECT * 已含·未建/坏 meta→[])
      const _selfSeedFacts = seedMetaToFactItems(companion.character_seed_meta, { friendNick: _friendNick });
      _factSnapshot = buildCanonicalFactSnapshot(companion, { userText, factTopics: _factTopics, personaFacts, userFacts: memories, selfSeedFacts: _selfSeedFacts });
      const _factGuard = analyzeFactGuard(userText, _factSnapshot, _factTopics);
      systemPrompt += _factGuard.hint;
      _factGuardSpans = _factGuard.unsupportedSpans;
    } catch (e) { log('warn', `[FactGuard] 注入失败 companion=${companion.id}: ${e.message}`); }
    // ⑤a（B2·D3 批2）：索图轮细则条件块 PHOTO_COND——复用上面 :714 已算的 photoIntent（同源 photo_intent.STRONG_PATTERNS·绝不重声明 regex）。
    // 文本回复路径命中索图=strong+esc≥2(拒图)/weak_photo_context（strong+esc<2 已走生图 return 到不了这）。触发面宁宽勿漏·漏检=只剩压缩底线。
    if (photoIntent.type === 'strong_photo_request' || photoIntent.type === 'weak_photo_context') systemPrompt += PHOTO_COND;
    // PR-3·B scene 守卫提示（B-min）：current_scene(裸 location 由 §6 注入)+current_scene_meta 经
    // parseSceneState 渲染进食态/待过渡的自然句（planning"别说成刚吃完"/transition_pending），
    // 🔴 绝不拼 JSON 原文；普通场景返回空串不刻板。fail-open。
    try { systemPrompt += buildSceneHint(companion.current_scene, companion.current_scene_meta); }
    catch (e) { log('warn', `[Scene] hint 注入失败 companion=${companion.id}: ${e.message}`); }
    ...
    // ④（2026-06-21）：hoist isFirstTurn·下方出站愧疚护栏复用（首轮 arc=normal·scrubConflictRedline 早返回不扫）
    let isFirstTurn = false;
    try {
      const _prior = getRecentHistory(msg.fromUser, botId, 6) || [];
      isFirstTurn = !_prior.some(m => m.direction === 'out');
      if (isFirstTurn) systemPrompt += buildFirstTurnHint(companion);
    } catch (e) { log('warn', `[Bot] firstTurn check failed: ${e.message}`); }
    // 🔴 topic_opener（2026-06-25 回库·停板B 二轮）：对话中她主动开新话头·只活在同步 reply 路径（有 userText）·
    //   绝不读沉默·绝不落库对话状态。🔴 主刀=每日预算（getTopicOpensToday/bumpTopicOpensToday·日界重置·非时间间隔），
    //   替旧 `_topicOpenerOpenedBySession` 进程级永不重置计数器（真根因）。dogfood 白名单门控·默认 OFF。
    try {
      if (topicOpenerEnabledFor(companion.id)
          && topicClosureSignal(recentTurns, userText, { isFirstTurn, arcActive: arcCtx.active, crisisLevel: _crisisLevel })) {
        const _openedToday = getTopicOpensToday(companion.id, todayKey);   // 今日已开次数（日界自愈·非时间）
        let _activeLoops = [];
        try { _activeLoops = listOpenLoops(companion.id, { status: 'open', limit: 8 }); } catch { /* fail-open */ }
        let _lifeStatesForTopic = [];
        try { _lifeStatesForTopic = getActiveLifeStates(companion.id); } catch { /* fail-open */ }
        const _openerHint = buildTopicOpenerHint(recentTurns, {
          works: getActiveCurrentWorks(companion.id),
          lifeStates: _lifeStatesForTopic,
          dailySchedule,
          openLoops: _activeLoops,
          affectionLevel: companion.affection_level,
        }, { alreadyOpenedCount: _openedToday });
        if (_openerHint) {
          systemPrompt += _openerHint;
          bumpTopicOpensToday(companion.id, todayKey);   // 🔴 每日预算 +1（日界重置·非时间·survive 重启）
          log('info', `[TopicOpener] 注入主动话头 hint companion=${companion.id} opened=${_openedToday + 1}/${TOPIC_OPEN_DAILY}`);
        }
      }
    } catch (e) { log('warn', `[TopicOpener] 注入失败（fail-open·不阻断回复）companion=${companion.id}: ${e.message}`); }
...
- 如果他坚持再聊几句，你就再陪一下下，但别忘了你也困了、也该睡了`;
    ...
    const genReplyOnce = () => _crisisLevel === 'high'
      ? buildCrisisReply(classifyCrisisSeverity(userText, _recentUserTexts))
      : generateReply(
        systemPrompt,
        history,
        userText,
    ...
    // 🔴 topic_opener 出站反愧疚/续话词强制扫（回库·停板B 二轮·绕 !inConflict·分段剥保主回复·仅白名单 companion 跑）
    if (topicOpenerEnabledFor(companion.id)) reply = scrubTopicOpenerRedline(reply, companion.id);
    // ④（2026-06-21）首轮破冰愧疚护栏（first-turn-only·纯词表·复用 A刀绕 !inConflict 强制扫——首轮
    // arc=normal·上面 scrubConflictRedline 早返回不扫愧疚=假绿陷阱）：命中首轮愧疚词 → drop 回退安全模板
    // （非 retry·防 LLM 重生又漂愧疚）。🔴 仅 isFirstTurn·绝不套普通对话。形状护栏经沙箱实证对首轮净负
    // （误杀健康多钩子破冰·0 真拦截）已砍·主防=词表 + buildFirstTurnHint 禁句。
    if (isFirstTurn && hitsFirstTurnGuiltWord(reply)) {
      log('warn', `[Moderation] 首轮破冰愧疚命中 → 回退安全模板 companion=${companion.id}`);
      reply = buildFirstTurnFallback(companion);
    }
    // PR-3 闭环兜底：本轮他抛的 canonical 无支持 claim（育英小学/父母职业等）若被确认式复述 → scrub
    // （否定/转 RP 放行）。把"凭空补全伪往事"开放问题缩成"本轮这几个 span 有没有被确认"的封闭检查。
    reply = scrubUnsupportedClaimConfirmation(reply, _factGuardSpans, companion.id);
    // PR-3.1·B 闭域 gate（防误伤·conflict/unsupported 才 fire·否定/RP 优先放行·fail-open）：
    // ① critical 槽位冲突确认(你爸是老师↔canonical出租车)→软纠正带真值 ② 无支持共同经历确认→软纠正。
    try {
      reply = scrubCriticalSlotContradiction(reply, userText, _factTopics, _factSnapshot);
      reply = scrubUnsupportedSharedTopicConfirmation(reply, _factTopics, _factSnapshot);
      reply = scrubSelfIdentityNumberDrift(reply, userText, companion);   // 批C·C2/G1：AI 自发年龄漂移出站门（AI 自发域·同排新第四门·单源 fact_age_scan）
    } catch (e) { log('warn', `[FactGuard] 出站 gate 失败 companion=${companion.id}: ${e.message}`); }
    // PR-3·B 场景瞬移出站兜底：current_scene 非中性 / 进食未完时，拦 reply 凭空"刚吃完X回来"(某存量案例)。
    reply = scrubSceneJump(reply, companion.current_scene, companion.current_scene_meta);
    ...
    // ── 防沉迷用量测量 co-located（必须在 recordUserReplied 之前·读到 prev=上一条入站）·fail-open ──
    try { touchUsageSession(companion); } catch {}
    ...
    // D1：整条 reply 至多 1 张（跨段硬上限·非每段 1）——预算随发出递减，用尽后续段只剥标记不发。
    let stickerBudget = allowStickers ? 1 : 0;
    for (let i = 0; i < segments.length; i++) {
      const segment = segments[i];
      // D1 预算 + D2/缺口① enabled（冷却中/开关关→出口硬 drop 幻觉标记·超额/畸形标记一律剥离不外漏）
      const { text: textOnly, stickers } = parseStickerMarkers(segment, { max: stickerBudget, enabled: allowStickers });
    ...
    // 🔴 防沉迷「连续使用 2h」动态提示（法规②·停板A 审过）：独立最后一条气泡（绕过出站 scrub 不被剥离），
    // 先写去重态再发（永不 spam·拍板点7），发送前 computeUsageNotice 已跑运行时红线 assert。全 fail-open：
    // 异常绝不吞主 reply。不落 saveConversationTurn（不污染 LLM 历史·特性非缺陷·拍板点9）。
    try {
      // isSleeping 交 computeUsageNotice 内部自查(safeIsSleeping·now=clock.now())→与 duration 同源，
      // 免"睡眠判据走真墙钟、时长判据走注入假钟"边界抖动(实现红队时序镜·low·prod无害但同源更稳)。
      const _notice = computeUsageNotice(companion, { crisisLevel: _crisisLevel });
      if (_notice) {
        markUsageNoticed(companion, _notice.noticeCount);
        await sendTyping(ctx, msg.fromUser, msg.contextToken);
        await sleep(randInt(1200, 2600));
        await sendAndRecord(ctx, msg.fromUser, _notice.text, msg.contextToken);
        log('info', `[AntiAddiction] 时长提示已发 companion=${companion.id} seq=${_notice.noticeCount}`);
      }
    } catch (e) { log('warn', `[AntiAddiction] 提示发送失败·fail-open companion=${companion.id}: ${e.message}`); }
...
async function handleBindCodeMessage(ctx, msg, botId) {
  ...
  try {
    ...
    await sendBindDisclosure(ctx, msg.fromUser, result.companionId, msg.contextToken);   // 🔴 07-15 合规①·AI 显著标识
...
// 🔴 07-15 合规 ①·入口 AI 显著标识：绑定成功后追加一条【系统口吻·第三人称】披露气泡（法规「显著标识」）。
// 走 sendAndRecord=绕 persona_guard(不撞 AI_SELF_DISCLOSURE·非「她」第一人称自称 AI)+留痕 wechat_messages。
// 角色名用 companion.name 动态插值(勿硬编"溪语")；companionId 缺(未建人设)时退第三人称泛称。fail-open 不阻塞绑定。
async function sendBindDisclosure(ctx, toUser, companionId, contextToken) {
  try {
    let name = '';
    if (companionId) { const { getCompanionById } = await import('./db.mjs'); name = getCompanionById(companionId)?.name || ''; }
    const roleRef = name ? `AI 虚拟角色「${name}」` : 'AI 虚拟角色';
    // 🔴 披露须为真话·锚 terms.html:31 口径：她实发=文字回复+AI 生成照片；**语音出站被 iLink 协议禁(静默丢弃)故不列**；
    // 贴纸是策展素材非"AI 生成"故不列。措辞与 terms「回复、照片…均由人工智能生成」一致。
    await sendAndRecord(ctx, toUser, `【系统提示】你正在与${roleRef}对话，对方并非真人，她的回复、照片等内容均由人工智能生成。`, contextToken);
  } catch (e) { log('warn', `[Bot] bind disclosure send failed (fail-open): ${e.message}`); }
}
