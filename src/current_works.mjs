...
import { WORKS_TENSE_LOCK } from './tense_lock_terms.mjs';   // A1: 时态锁 works 进行态·单源(权威)
...
export function worksConfig(env = process.env) {
  ...
  return {
    ...
    skillDaysMin: num('WORKS_SKILL_DAYS_MIN', 30), skillDaysMax: num('WORKS_SKILL_DAYS_MAX', 60),   // skill 比看书/手工长·一个月起步才告一段落归档
    dropProb: num('WORKS_DROP_PROB', 0.15),
  };
}

/** 🔴 灰度闸 WORKS_SKILL 默认关：gate buildWorkGenPrompt 的 skill 选项=闸关 gen prompt 不变=零 skill 生成=
 *  其它 skill 分支休眠=部署零变更先验（同会话纪律）。开闸后 LLM 才可能产出 skill 档案。 */
export function isSkillKindOn(env = process.env) {
  return /^(1|true|on|yes)$/i.test(env.WORKS_SKILL || '');
}
...
export function lifecycleDays(kind, startedAt, cfg = worksConfig()) {
  ...
  const [lo, hi] = k === 'book' ? [cfg.bookDaysMin, cfg.bookDaysMax]
    : (k === 'series' || k === 'anime' || k === 'game') ? [cfg.seriesDaysMin, cfg.seriesDaysMax]
      : k === 'skill' ? [cfg.skillDaysMin, cfg.skillDaysMax]
...
const PROGRESS_STAGES = {
  ...
  // 🔴 skill 挫折选 A：纯成就框架·早期 stage 承认笨拙("手忙脚乱")给真实感·绝不抱怨/求安慰·单调升不回退。
  skill:  ['刚开始学还手忙脚乱', '慢慢摸到点门道', '练得有模有样了', '已经挺像样了'],
};

/** kind × ratio → 渐进进度文案（确定性；ratio 单调↑则文案单调推进，绝不跳级/回退）。 */
export function progressStageNote(kind, ratio) {
  const k = ['book', 'series', 'anime', 'game', 'craft', 'skill'].includes(String(kind || '').toLowerCase())
...
export function buildScheduleWorksHint(works) {
  ...
  const verb = (k) => { const x = String(k || '').toLowerCase();
    return (x === 'series' || x === 'anime') ? '在追' : x === 'game' ? '在玩' : x === 'craft' ? '在做' : x === 'skill' ? '在学' : '在看'; };
  const lines = list.map(w => {
    const _k = String(w.kind || '').toLowerCase();
    const t = w.verify_status === 'generic' ? `一本${stripBrackets(w.title)}`
      : (_k === 'craft' || _k === 'skill') ? stripBrackets(w.title)
        : `《${stripBrackets(w.title)}》`;
    return `${verb(w.kind)}${t}${w.progress_note ? `（${w.progress_note}）` : ''}`;
  });
  return `
【她正在进行的事（来自档案 · 必须用真实条目）】她${lines.join('；')}。
今天日程里「读书/看剧/看番/打游戏/做手工/学技能」类活动**必须引用上面这些真实条目的原名**（她在看哪本就写哪本、在学什么就写什么），**绝不另编新书名/剧名**；进度自然延续（昨天看到一半→今天接着看）。
...
function renderWork(w) {
  ...
  if (kind === 'skill') {                                   // 🔴 skill 在 craft/skip 前判（否则落 craft 渲染成"手上在吉他"）
    return `在学${stripBrackets(w.title)}${prog}`;           // "在学吉他（练得有模有样了）"
  }
 ...
 */
export function buildWorksPromptHint(works, opts = {}) {
  const list = (Array.isArray(works) ? works : []).filter(w => w && w.title);
  if (!list.length) return '';
  const line = list.map(renderWork).join('；');
  // D（停板B·PROACTIVE_ROUTINE_COHERENCE·opts.lockTense 仅 proactive+闸开+age≥18 传真=reply 不传=字节一致）：
  // 锁进行态——治"worksHint'最近在看一本推理小说'被 LLM 翻成'刚把推理小说看完了'"的时态漂移（现象1 支因）。
  const tenseLock = opts.lockTense ? WORKS_TENSE_LOCK : '';
...
- **别的作品名——哪怕你前面对话里提到过——现在都当已经看完/翻完了**：他问起那些旧名，就自然带过（"啊那本早看完啦"），**绝不接着说它的进度或剧情**（接着编=继续虚构），也别否认说自己没提过。要聊新书只从上面这些里出。${tenseLock}`;
}
...
export function worksSceneSeed(work) {
  ...
  if (kind === 'skill') {
    return `此刻你在学/在练的「${t}」——first-person POV 低头俯拍练习的场景与器材（如吉他指板、谱子、菜板、瑜伽垫、画纸），`
      + `**不出现脸、不出现人**。caption 像随手分享你在练这个${prog || '，别太用力'}。`;
  }
...
export function buildWorkArchiveMemory(work, dropped, now = new Date()) {
  ...
  } else if (String(work?.kind || '').toLowerCase() === 'skill') {
    const t = stripBrackets(work?.title);
    content = dropped ? `${monthLabel}学${t}学了半截放下了` : `${monthLabel}学了阵${t}${note || '，算入门了'}`;
...
export function buildWorkGenPrompt(companion, { existingTitles = [], recentTitles = [] } = {}) {
  ...
  // 🔴 灰度闸 WORKS_SKILL：闸关时 skillClause/kindEnum 退回旧值=gen prompt 字节一致旧行为（零 skill 生成·零变更先验）。
  const skillOn = isSkillKindOn();
  const skillClause = skillOn
    ? '，或在学的一种技能(skill·如吉他/书法/做菜/健身/画画/语言)——🔴skill 是她【持续在练的一种能力】、不是做一件东西(和 craft 手工成品区分)，是【她自己学的、与当前聊天的人无关、不是社交八卦】，按她性格选(文静→书法/语言/画画，外向活泼→吉他/健身/舞蹈)'
    : '';
  const kindEnum = skillOn ? 'book|series|anime|game|craft|skill' : 'book|series|anime|game|craft';
  const skillTitleNote = skillOn ? 'skill 的 title 填技能名(如"吉他""书法"，不加"学"字)、creator 填 null。' : '';
...
生成一件她最近在看的**真实存在**的书/剧/番/游戏，或在做的手工(craft)${skillClause}。书优先填作者，剧/番填出品方。${skillTitleNote}
返回 {"kind":"${kindEnum}","title":"真实存在的作品名(不加书名号《》、不要编)","creator":"作者或出品方(craft 填 null)","genre":"题材如 推理/科幻/治愈","progressNote":"进度一句话如 看到第三章"}`;
