...
import * as clock from './clock.mjs';   // context_token 24h 窗口 TTL=业务时间·走可注入时钟(生产=真实时间·测试可拨)
import { isLifeFoundationOn, deriveLifeFoundation, extractHometownProvince } from './life_foundation.mjs';   // v1.23 静态生活地基（确定性派生·灰度闸）
import { scoreMemoryForRecall } from './memory_decay.mjs';   // 批D·件③b：D2-1 decay 打分（零依赖叶子·db→leaf 无环）
import { EXPIRE_DAYS, EXPIRE_TIMELY_KINDS, FOLLOWUP_CAP, isDueExpired } from './open_loops_lifecycle.mjs';   // 批D·件⑤：open_loops 生命周期单源（零依赖叶子·db→leaf 无环）
import { migrateArcToPulses, negativeEmotionSum, ENGINE_CALM_FLOOR, emotionEngineOn, reappraise } from './emotion_engine.mjs';   // 批E·D4/E0b：迁移折算 + engine_calmed 收敛 + 再评价（零依赖叶子·db→leaf 无环）

const DB_PATH = process.env.DB_PATH || path.resolve(process.cwd(), 'data/bot.db');
// 默认人设名（散落硬编码单源化·可 env 覆写做白标）。schema DEFAULT 仅兜底，createCompanion 均显式传入。
export const DEFAULT_COMPANION_NAME = process.env.DEFAULT_COMPANION_NAME || '溪语';
// 确保 data 目录存在
...
export function getDb() {
  if (!db) {
    ...
    migrateSocialCircle();    // 社交圈 Step1 companion_social_circle（id-seeded 闺蜜锚 + 社交碎事·独占写）
    migrateEmotionPulses();   // 批E·D4 情绪引擎脉冲表 + E0b 迁移留痕列（闸 OFF 时表存在但零读写=字节一致）
  }
  return db;
}
...
// ─── 批E·D4 情绪引擎脉冲层（E0c 闸后 dark：闸 OFF 时表存在但零读写=旧行为字节一致）─────
// companion_emotion_pulses = 引擎情绪脉冲事实源（OCC 事件评价 + E0b 存量迁移写入·三段半衰读时算）。
// 🔴 emotion CHECK 焊死 5 正 5 负枚举——contempt 注入即约束报错（铁线"轻蔑轨道结构排除"落到 DB 层）。
function migrateEmotionPulses() {
  getDb().exec(`
    CREATE TABLE IF NOT EXISTS companion_emotion_pulses (
      id            INTEGER PRIMARY KEY AUTOINCREMENT,
      companion_id  INTEGER NOT NULL REFERENCES companions(id) ON DELETE CASCADE,
      emotion       TEXT    NOT NULL CHECK(emotion IN (
                      'joy','gratitude','pride','hope','relief',
                      'worry','upset','sadness','annoyance','guilt')),
      intensity     REAL    NOT NULL,           -- base 强度 0-100（at 时刻值·衰减从此算）
      at            TEXT    NOT NULL,           -- 脉冲起始 ISO
      importance    REAL    NOT NULL DEFAULT 1, -- → 半衰乘数
      unresolved    INTEGER NOT NULL DEFAULT 0, -- → 反刍减速
      source        TEXT    NOT NULL DEFAULT 'event',  -- event | arc_migration
      event_id      TEXT,                       -- 同源加速锚（冲突事件 / migration_event_id）
      migrated_from TEXT,                       -- 迁移审计：原 arc_state（仅迁移行）
      created_at    TEXT    NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_emotion_pulses
      ON companion_emotion_pulses(companion_id, at DESC);
  `);
  addColIfMissing('companions', 'emotion_migrated_at', 'TEXT');  // E0b 幂等标（companion 级·已迁则跳过）
  addColIfMissing('companion_relationship_events', 'resolve_note', 'TEXT');  // resolve 留痕（迁移 / engine_calmed·不删档）
  addColIfMissing('companions', 'engine_calmed_count', 'INTEGER DEFAULT 0');  // engine_calmed 触发计数（digest 观察·dogfood 校准 floor）
}

/** 写一条引擎情绪脉冲（OCC 评价 / arc 迁移共用入口）。 */
export function insertEmotionPulse(companionId, pulse = {}, now = new Date()) {
  const {
    emotion, intensity, at = now.toISOString(), importance = 1, unresolved = false,
    source = 'event', event_id = null, migrated_from = null,
  } = pulse;
  return getDb().prepare(`
    INSERT INTO companion_emotion_pulses
      (companion_id, emotion, intensity, at, importance, unresolved, source, event_id, migrated_from, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    companionId, String(emotion), Number(intensity), String(at), Number(importance),
    unresolved ? 1 : 0, String(source), event_id, migrated_from, now.toISOString(),
  ).lastInsertRowid;
}

/** 读引擎情绪脉冲（默认最近 200 条·shape 直喂 emotion_engine.decayEmotionIntensity）。 */
export function getEmotionPulses(companionId, { sinceMs = null, limit = 200 } = {}) {
  const rows = getDb().prepare(`
    SELECT emotion, intensity, at, importance, unresolved, source, event_id, migrated_from
    FROM companion_emotion_pulses WHERE companion_id = ?
    ORDER BY at DESC LIMIT ?
  `).all(companionId, Math.max(1, Math.min(1000, limit | 0)));
  return rows
    .map(r => ({ ...r, unresolved: !!r.unresolved }))
    .filter(r => sinceMs == null || new Date(String(r.at).replace(' ', 'T')).getTime() >= sinceMs);
}

/** E0b 幂等标读取（null=未迁移）。 */
export function getEmotionMigratedAt(companionId) {
  const row = getDb().prepare('SELECT emotion_migrated_at FROM companions WHERE id = ?').get(companionId);
  return row?.emotion_migrated_at || null;
}

/**
 * E0b 存量 arc → 引擎迁移（开闸首次为真处 lazy 触发·原子事务·幂等）。
 * 纯折算走 emotion_engine.migrateArcToPulses；本函数只管 DB 副作用（写脉冲/清 arc/留痕/幂等标）。
 * 🔴 scar 直接清除【不扣 trust】（SCAR_TRUST_PENALTY 随退役一并作废·本函数不碰 trust）。
 * @returns { migrated, from, pulses, migrationEventId, count } | { migrated:false, reason:'already' }
 */
export function migrateArcToEngine(companionId, now = new Date()) {
  if (getEmotionMigratedAt(companionId)) return { migrated: false, reason: 'already', pulses: [] };
  const { arc_state, arc_state_changed_at } = getArcState(companionId);
  const stateChangedMs = arc_state_changed_at
    ? new Date(String(arc_state_changed_at).replace(' ', 'T')).getTime() : now.getTime();
  const { pulses, clearScar } = migrateArcToPulses(arc_state, stateChangedMs, now);
  const nowIso = now.toISOString();
  const migrationEventId = `mig_${companionId}_${now.getTime()}`;   // 唯一·cold 双脉冲共享（同源加速锚）

  getDb().transaction(() => {
    for (const p of pulses) {
      insertEmotionPulse(companionId, {
        ...p, at: nowIso, source: 'arc_migration', event_id: migrationEventId, migrated_from: arc_state,
      }, now);
    }
    // open/repairing 事件 → resolved（留痕·不删档·审计）
    getDb().prepare(`
      UPDATE companion_relationship_events
      SET repair_status = 'resolved', resolved_at = ?, resolve_note = 'migrated_to_engine'
      WHERE companion_id = ? AND repair_status IN ('open','repairing')
    `).run(nowIso, companionId);
    setArcState(companionId, 'normal', nowIso);   // arc 清 normal（scar 一并清·重置 changed_at）
    getDb().prepare('UPDATE companions SET emotion_migrated_at = ? WHERE id = ?').run(nowIso, companionId);  // 幂等标
  })();

  return { migrated: true, from: arc_state, clearScar, pulses, migrationEventId, count: pulses.length };
}

/**
 * engine_calmed 收敛（E3 §3.3·接缝唯一新机制·防"hint 已平静而 arc 还挂 hurt"表里分裂）。
 * 闸 ON 时若 arc∈{hurt,cold} 且【引擎同源负面】已衰减至 <ENGINE_CALM_FLOOR → 事件 resolved(note=engine_calmed)、态回 normal。
 * 🔴 方向=降级平静（读引擎强度非读沉默·与 T12 同族合法）；收敛的是账面不是感受（脉冲仍自然衰减）；
 *   repairing 不走此收敛（修复要互动完成·保留仪式感）；无自增强环（事件→引擎→calmed→resolved→终）。
 * @returns { calmed, from, sum?, eventId? } | { calmed:false, reason }
 */
export function engineCalmedCheck(companionId, now = new Date()) {
  if (!emotionEngineOn(companionId)) return { calmed: false, reason: 'gate_off' };   // 字节安全：闸 OFF 绝不收敛
  const { arc_state } = getArcState(companionId);
  if (arc_state !== 'hurt' && arc_state !== 'cold') return { calmed: false, reason: 'arc_not_hurt_cold', from: arc_state };
  const ev = getOpenRelationshipEvent(companionId);   // 活跃冲突事件=同源锚
  if (!ev) return { calmed: false, reason: 'no_open_event', from: arc_state };
  const sum = negativeEmotionSum(getEmotionPulses(companionId).filter(p => p.event_id === String(ev.id)), now);  // 🔴 只看同源
  if (sum >= ENGINE_CALM_FLOOR()) return { calmed: false, reason: 'above_floor', from: arc_state, sum };
  const nowIso = now.toISOString();
  getDb().transaction(() => {
    getDb().prepare(`UPDATE companion_relationship_events SET repair_status = 'resolved', resolved_at = ?, resolve_note = 'engine_calmed' WHERE id = ?`).run(nowIso, ev.id);
    setArcState(companionId, 'normal', nowIso);   // 降级平静（态回 normal）
    getDb().prepare('UPDATE companions SET engine_calmed_count = COALESCE(engine_calmed_count, 0) + 1 WHERE id = ?').run(companionId);  // digest 计数
  })();
  return { calmed: true, from: arc_state, sum, eventId: ev.id };
}

/**
 * 再评价写回（Phase4 接缝·同源加速/换视角落库）。读脉冲→纯 reappraise→UPDATE 变更行（强度/at/unresolved）。
 * @param trigger { type:'resolve'|'heard'|'self'|'dilute', event_id?, isNextDay?, roll? }
 * @returns 变更条数
 */
export function reappraiseEngine(companionId, trigger = {}, now = new Date()) {
  const rows = getDb().prepare(
    'SELECT id, emotion, intensity, at, importance, unresolved, source, event_id, migrated_from FROM companion_emotion_pulses WHERE companion_id = ?',
  ).all(companionId).map(r => ({ ...r, unresolved: !!r.unresolved }));
  if (!rows.length) return 0;
  const updated = reappraise(rows, trigger, now);
  let n = 0;
  getDb().transaction(() => {
    for (let i = 0; i < rows.length; i++) {
      const b = rows[i], a = updated[i];
      if (a.intensity !== b.intensity || a.at !== b.at || a.unresolved !== b.unresolved) {
        getDb().prepare('UPDATE companion_emotion_pulses SET intensity = ?, at = ?, unresolved = ? WHERE id = ?')
          .run(a.intensity, a.at, a.unresolved ? 1 : 0, b.id);
        n++;
      }
    }
  })();
...
// ─── 社交圈 Step1（companion_social_circle）──────────────────────────────────────────────────
// 一行/companion：stable_contacts（id-seeded 闺蜜锚·稳定不蹦新名）+ recent_events（社交碎事·14天窗）。
// 🔴 故意不进 ALLOWED_FIELDS / 不开通用 PATCH——生命周期任务(refreshSocialCircle)独占写，防 dashboard
// 伪造社交关系（同 current_works / life_state / arc_state 范式）。Step1 只用本表·不写 companion_memories。
function migrateSocialCircle() {
  getDb().exec(`
    CREATE TABLE IF NOT EXISTS companion_social_circle (
      companion_id         INTEGER PRIMARY KEY REFERENCES companions(id) ON DELETE CASCADE,
      stable_contacts_json TEXT,    -- [{nickname, relation, her_life_blob, knows_user:0}]（id-seeded·稳定）
      recent_events_json   TEXT,    -- [{contact, text, created_at}]（社交碎事·过社交 guard·14天窗）
      updated_at           TEXT
    );
  `);
}

/** 读社交圈（解析 JSON·容错返 {stable_contacts:[], recent_events:[]}）。 */
export function getSocialCircle(companionId) {
  const row = getDb().prepare(`SELECT stable_contacts_json, recent_events_json FROM companion_social_circle WHERE companion_id = ?`).get(companionId);
  if (!row) return null;
  const parse = (s) => { try { const v = JSON.parse(s || '[]'); return Array.isArray(v) ? v : []; } catch { return []; } };
  return { stable_contacts: parse(row.stable_contacts_json), recent_events: parse(row.recent_events_json) };
}

/** 写社交圈（独占写·不进 ALLOWED_FIELDS）。recent_events 的 text 已在 social_circle 层过 guard。 */
export function upsertSocialCircle(companionId, { stable_contacts = [], recent_events = [] } = {}) {
  return getDb().prepare(`
    INSERT INTO companion_social_circle (companion_id, stable_contacts_json, recent_events_json, updated_at)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(companion_id) DO UPDATE SET stable_contacts_json = excluded.stable_contacts_json,
      recent_events_json = excluded.recent_events_json, updated_at = excluded.updated_at
  `).run(companionId, JSON.stringify(stable_contacts), JSON.stringify(recent_events), new Date().toISOString()).changes;
}
...
function migrateOpenLoops() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS companion_open_loops (
      ...
      status           TEXT NOT NULL DEFAULT 'open'
                       CHECK(status IN ('open','resolved','stale','dismissed','expired')),
  ...
  // 🔴 批D·件⑤：注入计数（决议 297⑦·满 FOLLOWUP_CAP 次 → 强制 expired）
  addColIfMissing('companion_open_loops', 'follow_up_count', 'INTEGER DEFAULT 0');
  // 🔴 批D·件⑤（决议 297⑧）：老表 status CHECK 无 'expired'（SQLite CHECK 不可 ALTER）→ 表重建加入。
  //   先 addColIfMissing 保列齐、再重建 copy 全列（循 poll_state/companion_memories 表重建先例）。
  const _loopSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='companion_open_loops'").get()?.sql || '';
  if (!_loopSql.includes("'expired'")) {
    db.exec(`
      PRAGMA foreign_keys=OFF;
      BEGIN;
      CREATE TABLE companion_open_loops_new (
        id               INTEGER PRIMARY KEY AUTOINCREMENT,
        companion_id     INTEGER NOT NULL REFERENCES companions(id) ON DELETE CASCADE,
        title            TEXT    NOT NULL,
        due_at           TEXT,
        ...
        created_at       TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at       TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
        loop_kind        TEXT DEFAULT 'user_said',
        promise_payload  TEXT,
        follow_up_count  INTEGER DEFAULT 0
      );
      INSERT INTO companion_open_loops_new
        (id, companion_id, title, due_at, emotional_weight, expected_followup, status,
         source_message_id, resolved_at, resolved_text, followed_up_at, created_at, updated_at,
         loop_kind, promise_payload, follow_up_count)
        SELECT id, companion_id, title, due_at, emotional_weight, expected_followup, status,
               source_message_id, resolved_at, resolved_text, followed_up_at, created_at, updated_at,
               loop_kind, promise_payload, COALESCE(follow_up_count, 0)
        FROM companion_open_loops;
      DROP TABLE companion_open_loops;
      ALTER TABLE companion_open_loops_new RENAME TO companion_open_loops;
      CREATE INDEX IF NOT EXISTS idx_loops_companion_status_due
        ON companion_open_loops(companion_id, status, due_at);
      COMMIT;
...
function initSchema() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS companions (
      ...
      character_seed_meta TEXT,            -- PR-3.2 sidecar：user_character_seed+seed_processing(A)/character_core+values_core+seed_alignment(B)；🔴只 processSeedInput/saveCharacterSeedMeta 写·不进 ALLOWED_FIELDS 防绕过漏斗
      ...
      current_scene_meta TEXT,            -- PR-3·B sidecar：scene 结构化状态 JSON（activity/food_state/transition_pending/established_at/last_confirmed_at/source）；current_scene 保持裸 location 契约
      ...
      education TEXT,
      secrets TEXT,
      -- v1.23 静态生活地基（LIFE_FOUNDATION·系统派生·🔴非 ALLOWED_FIELDS=用户/dashboard 改不了）
      home_city TEXT,            -- 现居城市（具体·从城市池派生·跨对话恒定）
      life_district TEXT,        -- 城内片区（城东/城西…）
      life_field_idx INTEGER,    -- 专业/职业槽位 0..5（措辞按 live life_identity 切池组合·防漂移）
    ...
    -- 批E·E5④：入站消息计数热路径索引（_countInboundSince 的 from_user+direction 等值前缀）。
    -- 无此索引→arc time-tick 带 open 冲突事件时全表扫 wechat_messages（大表·全表扫 p95 崖/
    -- 30min 批 23.7s）；建后 seek 到该用户 in 分区→崖消。created_at 收尾列助有序/范围收敛。
    CREATE INDEX IF NOT EXISTS idx_wechat_messages_from_dir_time
      ON wechat_messages(from_user, direction, created_at);
...
function migrateCompanions() {
  const cols = [
    ...
    ['height',                'INTEGER DEFAULT 165'],
    ['clothing_style',        "TEXT DEFAULT '甜美'"],
    ['home_city',             'TEXT'],           // v1.23 静态生活地基（系统派生·非 ALLOWED_FIELDS）
    ['life_district',         'TEXT'],
    ['life_field_idx',        'INTEGER'],
    ['personality_tags',      'TEXT DEFAULT \'["温柔","体贴"]\''],
    ['mbti',                  'TEXT'],
    ['introvert_level',       'INTEGER DEFAULT 5'],
    ['intimacy_level',        "TEXT DEFAULT '慢慢熟悉'"],
    ...
    ['user_call_her_as',      'TEXT'],
    ['forbidden_topics',      "TEXT DEFAULT '[]'"],
    ['character_seed_meta',   'TEXT'],   // PR-3.2 sidecar（不进 ALLOWED_FIELDS·只 saveCharacterSeedMeta 写）
    ['updated_at',            'DATETIME DEFAULT CURRENT_TIMESTAMP'],
    // 本轮新增字段 ↓
    ['memory_enabled',        'INTEGER DEFAULT 1'],
    ['current_mood',          "TEXT DEFAULT '平静'"],
    ...
    ['current_scene',         "TEXT DEFAULT '在家'"],
    ['current_scene_meta',    'TEXT'],   // PR-3·B sidecar scene 结构化状态（裸 current_scene 契约不变）
    ['body_events_today',     'TEXT'],   // P2→P1 即时状态锚定：今日已声明身体态 JSON {showered:{at},washed_hair,woke,dreamed}；00:30 清
    ['scene_history',         "TEXT DEFAULT '[]'"],
    ['backstory',             'TEXT'],
    ['family_background',     'TEXT'],
    ['education',             'TEXT'],
    ...
    // 完整作息/日程系统：身份作息轴（与 role_title 人设风味正交）。NULL→按 age 兜底推导
    // （routine_profiles.resolveIdentity），存量零破坏；显式值见 IDENTITIES / subforms。
    ['life_identity',         'TEXT'],   // highschool(冻结存量兼容层·仅服务已存<18)|college|worker|exam_prep|fresh_grad
    ['life_subform',          'TEXT'],   // worker:nine_to_five|996|shift / fresh_grad:thesis|intern|jobhunt / exam_prep:fulltime|in_school
    // 🔴 topic 主动话头·每日预算（停板B 二轮·治"进程级永不重置"瓶颈）：今日已开次数 + 归属日（日界重置·非时间）。
    ['topic_opens_today',     'INTEGER DEFAULT 0'],
    ['topic_opens_date',      'TEXT'],
    // 🔴 防沉迷「连续使用 2h 提示」（法规②·停板A 审过·anti_addiction.mjs）：全 epoch ms。
    // 由 touchUsageSession 每条入站维护 anchor；computeUsageNotice 按锚点自愈去重（notice_last<anchor→旧计数作废）。
    ['usage_session_anchor_at', 'INTEGER'],           // 当前连续会话起点(epoch ms)
    ['usage_notice_last_at',    'INTEGER DEFAULT 0'],  // 上次提醒时刻(epoch ms)
    ['usage_notice_count',      'INTEGER DEFAULT 0'],  // usage_notice_last_at 所属会话内已提醒次数
  ];
...
function migrateConfessionFields() {
  ...
  // A刀（2026-06-20）：away_probe 一离开周期一次记账（epoch 秒）。> last_user_reply_at 即本周期已探。
  addColIfMissing('companions', 'last_away_probe_at', 'INTEGER');
}
...
export const ALLOWED_FIELDS = new Set([
  ...
  'current_scene', 'current_scene_meta', 'scene_history', 'backstory', 'family_background', 'education', 'secrets',
  'voice_style', 'voice_speed', 'chat_modes', 'chat_mode_active',
  'temperature', 'max_tokens', 'top_p',
]);

// ─── v1.22 voice security：voice_style 受控枚举 + voice_speed clamp（写入侧统一·覆盖 create/update 全路径）──
// 背景:voice_style 裸拼进 system prompt(companion.mjs:284)=prompt 注入面;voice_speed 越界喂坏 TTS(azure rate%)。
// 修法:写入侧把 voice_style 强制成枚举内值(非法→DEFAULT·"受控值"=注入 payload 根本入不了库)、
//      voice_speed 夹到 [0.5,2.0](=minimax 最窄 provider 区间·任何 provider 安全·DEFAULT 1.0 居中)。
// 🔴 枚举集覆盖所有现存在用值(温柔×16 / 甜美×1·prod 核实)+DEFAULT → 老 companion 迁移不落空/不被拒。
export const VOICE_STYLE_DEFAULT = '温柔';
export const VOICE_STYLE_ENUM = new Set([
  '温柔', '甜美',                                          // 现存在用(prod 核实·必须覆盖)
  '活泼', '知性', '御姐', '清冷', '元气', '慵懒', '软糯', '磁性',   // 常用档(供未来前端选择器·可审增删)
]);
export const VOICE_SPEED_DEFAULT = 1.0;
export const VOICE_SPEED_MIN = 0.5;
export const VOICE_SPEED_MAX = 2.0;
export function coerceVoiceStyle(v) {
  const s = (v == null ? '' : String(v)).trim();
  return VOICE_STYLE_ENUM.has(s) ? s : VOICE_STYLE_DEFAULT;   // 非枚举(含任意注入 payload)→回退 DEFAULT
}
export function clampVoiceSpeed(v) {
  const n = Number(v);
  return Number.isFinite(n) ? Math.max(VOICE_SPEED_MIN, Math.min(VOICE_SPEED_MAX, n)) : VOICE_SPEED_DEFAULT;
}

// ─── v1.22 persona_prompt 注入硬化（写入侧长度上限·a 方案）──────────────────────
// persona_prompt 裸拼进 system prompt(companion.mjs:582)=同源注入面·但自由文本不能枚举(枚举=砍产品),
// 只能硬化(定界+结构化在 companion.mjs·概率缓解)。写入侧加长度上限=确定性挡"超长注入灌入"·零误伤合法人设
// (prod 现存 max 285 字·600 留一倍余量)。🔴 不做内容黑名单(d):会误伤"她说话冲常说你必须/腹黑爱无视规矩"
// 这类合法人设·硬非法线由代码层 moderate/minor 兜。
export const PERSONA_PROMPT_MAXLEN = 600;
export function clampPersonaPrompt(v) {
  return v == null ? null : String(v).slice(0, PERSONA_PROMPT_MAXLEN);   // 截断超长(纯长度·不碰内容=零误伤)
}
...
function buildUpsertFields(data) {
  ...
  for (const [k, v] of Object.entries(data)) {
    ...
    else if (k === 'voice_style') values.push(coerceVoiceStyle(v));   // v1.22 voice security:受控枚举(注入入不了库)
    else if (k === 'voice_speed') values.push(clampVoiceSpeed(v));    // v1.22 voice security:clamp [0.5,2.0]
    else if (k === 'persona_prompt') values.push(clampPersonaPrompt(v));  // v1.22 persona_prompt 硬化:长度上限 ≤600
 ...
 *   - 填了生日 < 18 → canNsfw=false（未成年·仍强制 NSFW=0）
 *   - 填了生日 >= 18 → canNsfw=true
 * 🔴 v1.23 child-safety 步④：阈值 16→18（原 age>=16 与 isMinor=age<18 自相矛盾·16-17 未成年却 canNsfw=true）。
 *   当前此函数全库 0 消费者（死代码·无 live 风险）·趁没接线前堵雷·与法定 18 对齐。
 */
export function getUserAgeStatus(accountId) {
  ...
  const canNsfw = !ageKnown || age >= 18;   // 🔴 v1.23 child-safety 步④：16→18（与 isMinor=age<18 对齐·堵 16-17 未成年 canNsfw 雷）
...
// 🔴 2026-07-03 GPT·断流通(不可复用为头像源)：按 file_name 取 preset(含 enabled)·供 /avatar/select-preset 校验——
//   disabled(enabled=0) preset 即便磁盘 webp 还在(物理清理=youth-anchor 第二步)也不能被设成头像。单一来源 reader。
export function getAvatarPresetByFileName(fileName) {
  return getDb().prepare('SELECT id, file_name, age_range, enabled FROM avatar_presets WHERE file_name = ?').get(fileName) || null;
}
...
export function matchAvatarPresets(companion, queryEmbedding, topN = 4) {
  const db = getDb();
  // 🔴 2026-07-03 GPT·fail-closed（未成年角色不能通过成年化渲染变安全·应被图像层【拒绝】）：
  //   非明确成年（含 null/''/NaN/<18）在选图层直接拒绝返空——不再"→college 基线"（换皮≠停·违反自家 fail-closed）。
  //   ⚠ 内联复刻 isClearlyAdult（db↔visual_identity 环依赖不能 import·visual_identity.mjs:13 import db）·
  //     youth-anchor 板将统一为单一 assertAdultAvatarEligible（全入口盘点·isAntiAddictionOn 单一导出教训）。
  const _age = Number(companion?.age);
  if (companion?.age == null || companion.age === '' || !Number.isFinite(_age) || _age < 18) return [];
  ...
  // 派生成年年龄段（已过 fail-closed·age 必 ≥18 有效）
  const age = _age;
  const targetAgeRange = age <= 23 ? 'college' : 'young_pro';
...
export function ensureCompanion(wechatUserId, botId) {
  ...
  if (!c) {
    const db = getDb();
    db.prepare(`INSERT INTO companions (user_id, bot_id, name, persona_prompt) VALUES (?, ?, ?, '')`).run(user.id, botId, DEFAULT_COMPANION_NAME);
...
export function createCompanion(wechatUserId, botId, data) {
  // 🔴 合规确定性底（《暂行办法》·路线A 永久禁止未成年人设新建）：新建 age<18 直接拒。
  //    db 写咽喉 = 任何 caller（route / 直接调 / 未来新入口）都守·route 的 applyCompanionAgeGuard 只做 400 友好报错，
  //    此处兜底防未来漏 guard（同 voice/persona/地址下沉咽喉智慧·像 minor_guard 确定性）。
  //    🔴 只挡【新建 INSERT】；已存<18 的冻结存量（未成年）= 冻结保护·其正常迭代更新走 updateCompanion（无 age floor）·不被此拦。
  if (data && data.age != null && data.age !== '') {
    const _age = Number(data.age);
    if (Number.isFinite(_age) && _age < 18) {
      const err = new Error('AI 角色年龄不得低于 18 岁');
  ...
  // v1.23 静态生活地基（LIFE_FOUNDATION·默认关）：创建后确定性派生现居城市/片区/学校职业槽位·存字段·跨对话恒定。
  // 🔴 dedicated UPDATE 写（非 buildUpsertFields/ALLOWED_FIELDS）=用户/dashboard 改不了（系统派生·a·同 safe_mode 范式）。
  // 🔴 home_city IS NULL 守门 = 永不重生成（固化）；老家 fact 只读取省份避开·绝不覆盖。
  try {
    if (isLifeFoundationOn()) {
      const _row = getCompanionById(newId);
      const _prov = extractHometownProvince(getPersonaFacts(newId));
      const _lf = deriveLifeFoundation(_row, { avoidProvince: _prov });
      db.prepare('UPDATE companions SET home_city=?, life_district=?, life_field_idx=? WHERE id=? AND home_city IS NULL')
        .run(_lf.home_city, _lf.life_district, _lf.life_field_idx, newId);
    }
  } catch (e) { console.error(`[createCompanion] life_foundation 派生跳过: ${e.message}`); }
  enforceMinorSafetyInvariant(newId);   // v1.23 child-safety 兜底（createCompanion 已拒<18·此为防御:若拒被绕过仍收紧）
  return getCompanionById(newId);
}

// v1.23 静态生活地基 批回填：现存 home_city 空的 companion 一次性确定性派生固化（懒生成会留空窗=要消灭的不稳定）。
// 🔴 LIFE_FOUNDATION 门控；home_city IS NULL 才填（永不覆盖=固化）；老家 fact 只读取省份避开·绝不动。维护者启用闸后显式跑一次。
export function backfillLifeFoundation() {
  if (!isLifeFoundationOn()) return { skipped: 'flag_off', filled: 0, total: 0 };
  const db = getDb();
  const rows = db.prepare('SELECT id, age, life_identity FROM companions WHERE home_city IS NULL').all();
  let filled = 0;
  for (const r of rows) {
    try {
      const prov = extractHometownProvince(getPersonaFacts(r.id));
      const lf = deriveLifeFoundation(r, { avoidProvince: prov });
      const info = db.prepare('UPDATE companions SET home_city=?, life_district=?, life_field_idx=? WHERE id=? AND home_city IS NULL')
        .run(lf.home_city, lf.life_district, lf.life_field_idx, r.id);
      if (info.changes) filled++;
    } catch (e) { console.error(`[backfillLifeFoundation] companion=${r.id} 跳过: ${e.message}`); }
  }
...
export function updateCompanion(id, data) {
  ...
  enforceMinorSafetyInvariant(id);   // v1.23 child-safety 写咽喉派生：age<18 → safe_mode=1 + 关系封顶朋友（绕不过）
  return getCompanionById(id);
}

// v1.23 child-safety 写咽喉派生（补闸⑥A·治本）：companion.age<18 → 强制 safe_mode=1 + 关系封顶"朋友"。
//   把"未成年保护"从【靠 safe_mode 单开关被设对】变成【age<18 的结构不变量】——createCompanion/updateCompanion
//   任何写后都跑一遍·任何入口(route/直调/导入)绕不过（dedicated UPDATE·非 ALLOWED_FIELDS·dashboard/用户改不了）。
//   🔴 只收紧(开 safe_mode·降关系)绝不放松：age>=18/未知一律不动（不自动关 safe_mode·解除只走 attestation 端点）。
//   根因②对症：原 safe_mode 与 age 完全解耦·无路径因 age<18 自动设 safe_mode（id=16 即此漏）。
const _MINOR_STAGE_RANK = ['陌生人', '朋友', '暧昧', '恋人', '深爱'];
export function enforceMinorSafetyInvariant(id) {
  const db = getDb();
  const row = db.prepare('SELECT age, safe_mode, relationship_stage FROM companions WHERE id = ?').get(id);
  if (!row) return { changed: false, changes: [] };
  if (row.age == null || row.age === '') return { changed: false, changes: [] };   // 🔴 未知 age 不动(Number(null)=0 会误判<18·必先挡·不误锁无龄·同"没填生日=按成年")
  const age = Number(row.age);
  if (!Number.isFinite(age) || age >= 18) return { changed: false, changes: [] };   // 🔴 只对确定 <18 生效
  const changes = [];
  if (Number(row.safe_mode) !== 1) {
    db.prepare('UPDATE companions SET safe_mode = 1 WHERE id = ?').run(id);
    changes.push('safe_mode=1');
  }
  if (Math.max(0, _MINOR_STAGE_RANK.indexOf(row.relationship_stage)) > _MINOR_STAGE_RANK.indexOf('朋友')) {
    db.prepare("UPDATE companions SET relationship_stage = '朋友' WHERE id = ?").run(id);
    changes.push(`relationship ${row.relationship_stage}→朋友`);
  }
  if (changes.length) console.warn(`[child-safety] 写咽喉派生 id=${id} age=${age}: ${changes.join(', ')}`);
  return { changed: changes.length > 0, changes };
}

// 一次性 backfill（补闸⑥A 配套·收编存量）：对所有 age<18 跑不变量（收编 id=16 的 safe_mode + 关系）。维护者部署后跑一次。
export function backfillMinorSafetyInvariant() {
  const db = getDb();
  const ids = db.prepare('SELECT id FROM companions WHERE age IS NOT NULL AND age < 18').all().map((r) => r.id);
  const fixed = [];
  for (const id of ids) { const r = enforceMinorSafetyInvariant(id); if (r.changed) fixed.push({ id, changes: r.changes }); }
  return { scanned: ids.length, fixed };
}

// ── P2→P1 即时状态锚定：今日已声明身体态（send 时累积·prompt 时读·00:30 清=绕开 turn 查询的 UTC 边界 bug）──
export function getBodyEventsToday(id) {
  try {
    const row = getDb().prepare('SELECT body_events_today FROM companions WHERE id = ?').get(id);
    const obj = JSON.parse(row?.body_events_today || '{}');
    return (obj && typeof obj === 'object' && !Array.isArray(obj)) ? obj : {};
  } catch { return {}; }
}
/** 合并今日身体态事件（once-per-day 覆盖为最新一次声明 at；🔴 不收吃饭=让位 food_state 守卫）。events: {type:{at}} */
export function mergeBodyEventsToday(id, events) {
  try {
    if (!events || typeof events !== 'object' || !Object.keys(events).length) return;
    const cur = getBodyEventsToday(id);
    const ALLOWED = new Set(['showered', 'washed_hair', 'woke', 'dreamed']);   // 🔴 吃饭不在内·让位 food_state
    for (const [k, v] of Object.entries(events)) {
      if (!ALLOWED.has(k) || !v || !v.at) continue;
      cur[k] = { at: Number(v.at) };
    }
    getDb().prepare('UPDATE companions SET body_events_today = ? WHERE id = ?').run(JSON.stringify(cur), id);
  } catch { /* fail-open：抽取/写库失败绝不影响发送 */ }
}
/** 次日 00:30 清今日身体态（同 resetScenesForNewDay·上海日界） */
export function clearBodyEventsTodayAll() {
  try { return getDb().prepare('UPDATE companions SET body_events_today = NULL WHERE body_events_today IS NOT NULL').run().changes; }
  catch { return 0; }
}

// ── 🔴 topic 主动话头·每日预算（停板B 二轮·治"进程级永不重置"）。日界=上海日历日 dateKey（非时间间隔/沉默时长）──
// 读：归属日 != 今日 → 视为 0（日界自愈·不依赖 cron 也对·防 cron-miss/重启跨日）。
export function getTopicOpensToday(id, dateKey) {
  try {
    const row = getDb().prepare('SELECT topic_opens_today, topic_opens_date FROM companions WHERE id = ?').get(id);
    if (!row || row.topic_opens_date !== dateKey) return 0;   // 新的一天 → 0
    return Number(row.topic_opens_today) || 0;
  } catch { return 0; }
}
// 增：归属日变了先归零再 +1（日界重置·原子在一条 UPDATE 内）。返回新计数。
export function bumpTopicOpensToday(id, dateKey) {
  try {
    const cur = getTopicOpensToday(id, dateKey);   // 已含日界自愈
    const next = cur + 1;
    getDb().prepare('UPDATE companions SET topic_opens_today = ?, topic_opens_date = ? WHERE id = ?').run(next, dateKey, id);
    return next;
  } catch { return 0; }
}
/** 次日 00:30 清今日 topic 开话头计数（同 body_events·上海日界·belt-and-suspenders，读侧已自愈） */
export function clearTopicOpensTodayAll() {
  try { return getDb().prepare('UPDATE companions SET topic_opens_today = 0 WHERE topic_opens_today != 0').run().changes; }
  catch { return 0; }
}
...
/**
 * PR-3.2：character_seed_meta 的【唯一授权写入口】（除 processSeedInput 编排外）。
 * 🔴 character_seed_meta 故意不进 ALLOWED_FIELDS——禁 admin/dashboard/通用 PATCH 直写，
 *   防绕过输入漏斗污染 self seed。只此函数 + 创建/重生成链可写。
 */
export function saveCharacterSeedMeta(companionId, meta) {
  const db = getDb();
  const json = (meta == null) ? null : (typeof meta === 'string' ? meta : JSON.stringify(meta));
  db.prepare('UPDATE companions SET character_seed_meta = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
    .run(json, companionId);
}
...
// 🔴 批D·件⑤：加下界（粗 SQL·-noDue 天=最大 tier 安全网·防捞起远古 loop）+ 按 kind 精滤 isDueExpired
//   （考试案：时效类过 N 天不再被 recall 路捞起催问·三处同源之一）。expired 状态本就被 status='open' 排除，
//   此为夜批未跑窗口的读侧兜底（读时现算·不依赖 cron 活着）。
export function listDueOpenLoops(companionId, { withinHours = 24 } = {}) {
  ...
  const rows = db.prepare(`
    ...
    WHERE companion_id = ? AND status = 'open'
      ...
      AND datetime(due_at) >= datetime('now', '-${EXPIRE_DAYS.noDue} days')
    ORDER BY emotional_weight DESC, due_at ASC
    LIMIT 5
  `).all(companionId, withinHours);
  const today = shanghaiDateKey(new Date());
  return rows.filter(l => !isDueExpired(l.due_at, today, l.loop_kind));
}
...
// 🔴 批D·件⑤：follow-up 计数 + 到 FOLLOWUP_CAP 强制 expired（决议⑦·提醒 CAP 次松手·同防沉迷 CAP 美学）。
//   计数单调递增；累计到顶即落 'expired'（终态·从此不再进 open 候选）=注入上限硬止损环。原子单 UPDATE。
export function markOpenLoopFollowedUp(loopId) {
  const db = getDb();
  db.prepare(`
    UPDATE companion_open_loops
    SET followed_up_at = CURRENT_TIMESTAMP,
        follow_up_count = COALESCE(follow_up_count, 0) + 1,
        status = CASE WHEN COALESCE(follow_up_count, 0) + 1 >= ${FOLLOWUP_CAP} THEN 'expired' ELSE status END,
        updated_at = CURRENT_TIMESTAMP
    WHERE id = ?
  `).run(loopId);
}
...
// 🔴 批D·件⑤：分层过期 → 'expired'（决议 297②/⑧·夜批 cron）。函数名沿用历史（markStale），效果=按
//   loop_kind 分层判过期后落【新独立状态 'expired'】（非旧 'stale'）。N-day 天数=open_loops_lifecycle 叶子
//   受控整数常量（三处同源之一：另两处=dueStatus 封顶 / listDueOpenLoops 下界+精滤）·拼进 SQL 安全（非用户输入）。
export function markStaleOpenLoops(companionId = null) {
  ...
  const timelyList = EXPIRE_TIMELY_KINDS.map(k => `'${k}'`).join(',');
  return db.prepare(`
    UPDATE companion_open_loops
    SET status = 'expired', updated_at = CURRENT_TIMESTAMP
    WHERE status = 'open' ${where}
      AND (
        (due_at IS NOT NULL AND loop_kind IN (${timelyList})
           AND datetime(due_at) < datetime('now', '-${EXPIRE_DAYS.timely} days'))
        OR (due_at IS NOT NULL AND (loop_kind IS NULL OR loop_kind NOT IN (${timelyList}))
           AND datetime(due_at) < datetime('now', '-${EXPIRE_DAYS.standard} days'))
        OR (due_at IS NULL AND datetime(created_at) < datetime('now', '-${EXPIRE_DAYS.noDue} days'))
      )
  `).run(...args).changes;
}
...
// 防御性钳位：importance 合法域 = companion_memories 的 CHECK(importance BETWEEN 1 AND 10)（本文件 schema）。
// 起因(chip③·2026-07-03)：月压缩批 plan_tasks 写 importance=15 撞 CHECK → INSERT throw → 吞进 WARN →
// 原条目永不压缩。双层防御：写入点钳到合法域 + 此处每个 importance→INSERT 入口再钳一次（单一出口）。
// 未指定(null/undefined/空)回落 5（默认，非"0"）；非有限(如 'abc')回落 5；越界四舍五入后钳 [1,10]。
...
export function saveMemory({ companionId, userId, memoryType, content, importance = 5, keywords = null, embedding = null, pinned = null, memorySource = null, memoryLayer = null, memoryWeight = null }) {
  ...
  const db = getDb();
  const imp = clampImportance(importance);   // chip③ 防御性钳位（防 CHECK(1-10) 违约 throw）
  const isPinned = pinned !== null ? (pinned ? 1 : 0) : (imp >= 7 ? 1 : 0);
  ...
  const vals = [companionId, userId, memoryType, content, imp, isPinned, kw, emb];
...
export function saveMemories(memories) {
  ...
  const tx = db.transaction(list => {
    for (const m of list) {
      // 🔴 隐私全口子：批量路径与 saveMemory 同过 filterForStorage（对话抽取 memory.mjs / 图片记忆 api.mjs
      //   均走本批量入口·此前漏过滤=手机号/住址级明文入库·PIPL 洞）。密码/证件级 store=false 整条跳过；
      //   手机号/住址脱敏后入库。content 再过 replaceUserWording（穿帮词单一出口·同 saveMemory）。
      const pf = filterForStorage(m.content);
      if (!pf.store) { console.warn(`[PrivacyFilter] saveMemories 拦截敏感内容 companion=${m.companionId}`); continue; }
      const content = replaceUserWording(pf.text, companionUserAlias(m.companionId));
      const imp = clampImportance(m.importance);   // chip③ 防御性钳位（同 saveMemory·防 CHECK 违约）
      ...
      stmt.run(m.companionId, m.userId, m.memoryType, memoryLayerOfType(m.memoryType), content, imp, pinned, kw, emb);
    }
  });
  tx(memories);
}
...
// 🔴 批D·件①（信任事故·route≠filter 二次咬中·维护者 拍先修）：召回硬过滤单一权威源。
//   archived/deleted/contradicted（memory_status≠active）+ 用户标"勿提"（do_not_mention=1）的记忆
//   绝不召回喂 LLM。rankMemoriesForRecall(memory_v2.mjs:93) 虽含此过滤但零 caller=死代码——真路
//   recallMemories/Semantic 从不经过它，必须【在活路 SQL WHERE 直接补】（验 filter 非验 route）。
//   NULL 兼容旧行（列默认 active/0，防御性容 NULL）。改此条 = 两活路同源一处。
const MEM_RECALL_ACTIVE_FILTER = "AND (memory_status IS NULL OR memory_status = 'active') AND (do_not_mention IS NULL OR do_not_mention = 0)";
...
export function recallMemoriesSemantic(companionId, userId, queryEmbedding, limit = 7) {
  ...
  const rows = db.prepare(`
    ...
    FROM companion_memories
    WHERE companion_id = ? AND user_id = ? AND embedding IS NOT NULL ${MEM_RECALL_ACTIVE_FILTER}
...
// 🔴 批D·件④（D2-5 bigram 召回修）：高频 gram 停用词表——剔除「今天命中一切含今天的记忆」类误命中
//   （design_D2「停用词表：今天/明天/我们 等高频 gram 剔除」）。仅收时间/代词/填充类高频 bigram·死文本单源。
const RECALL_STOPWORD_BIGRAMS = new Set([
  '今天', '明天', '昨天', '前天', '后天', '今晚', '明晚', '昨晚', '早上', '晚上',
  '我们', '你们', '他们', '她们', '咱们', '这个', '那个', '这些', '那些',
  '什么', '怎么', '这样', '那样', '现在', '时候', '一下', '一个', '一些', '一起',
  '已经', '还是', '就是', '可以', '自己', '知道', '觉得', '应该', '但是', '因为',
  '所以', '如果', '可能', '然后', '这么', '那么', '不是', '没有', '这里', '那里',
]);

/**
 * 召回关键词提取（🔴 批D·件④ D2-5）：中文走 2 字滑窗 bigram（剔停用词）+ 英文/数字 token（≥2）原样。
 * 修老病=中文整串当一个 keyword→`content LIKE '%整句%'`几乎永不命中（keyword 档形同虚设）。
 * 单源喂 keyword 档 LIKE + scoreMemoryForRecall 的 ctxBoost（期2 proactive 查询词同修可复用·1b#14）。
 * @returns {string[]} 去重·封顶 cap 条
 */
export function extractRecallKeywords(text, cap = 6) {
  const s = String(text || '');
  const out = [];
  const seen = new Set();
  const push = (w) => { if (w && !seen.has(w)) { seen.add(w); out.push(w); } };
  // 英文/数字 token（≥2）
  for (const w of s.replace(/[^a-zA-Z0-9]/g, ' ').split(/\s+/)) {
    if (w.length >= 2) push(w.toLowerCase());
  }
  // CJK 连续段 → 2 字滑窗 bigram（单字段跳过·噪声大）
  for (const run of s.match(/[一-龥]+/g) || []) {
    for (let i = 0; i + 2 <= run.length; i++) {
      const bg = run.slice(i, i + 2);
      if (!RECALL_STOPWORD_BIGRAMS.has(bg)) push(bg);
    }
  }
...
export function recallMemories(companionId, userId, currentMessage, limit = 7) {
  const db = getDb();
  // 提取召回关键词（🔴 批D·件④ D2-5：CJK bigram + 停用词·同源喂 keyword 档 LIKE + ctxBoost）
  const keywords = extractRecallKeywords(currentMessage);

  // 第一档：pinned=1 永远候选
  const pinnedRows = db.prepare(`
    SELECT * FROM companion_memories
    WHERE companion_id = ? AND user_id = ? AND pinned = 1 ${MEM_RECALL_ACTIVE_FILTER}
    ORDER BY importance DESC, created_at DESC
    LIMIT 5
  `).all(companionId, userId);
  ...
  if (keywords.length > 0) {
    ...
    keywordRows = db.prepare(`
      SELECT * FROM companion_memories
      WHERE companion_id = ? AND user_id = ? AND (${conds}) ${MEM_RECALL_ACTIVE_FILTER}
      ORDER BY importance DESC, created_at DESC
      LIMIT ?
  ...
  // 第三档：高 importance 兜底候选（仅 pinned+keyword 未占满时取·不再按 imp 预切到 fill·占位由下方 decay 打分裁决）
  const needFill = limit - pinnedRows.length - keywordRows.length > 0;
  const topRows = needFill
    ? db.prepare(`
        SELECT * FROM companion_memories
        WHERE companion_id = ? AND user_id = ? ${MEM_RECALL_ACTIVE_FILTER}
        ORDER BY importance DESC, created_at DESC
        LIMIT ?
  ...
  // 🔴 批D·件③b(维护者 拍 A·原地升级)：候选池(pinned+keyword+topRows)改用 D2-1 decay 打分重排后切顶，
  //   不再靠 imp DESC 定最终序——修设计缺陷#1「打分无时间项→远期琐碎压近期状态」：远期 auto-pin(imp7)
  //   读侧褪色(件③a ignoreAutoPin)掉出前 N·近期语境约定登顶。ctxWords=当前消息关键词(件④ 将升 bigram)。
  //   一次一变量=仅换排序键：候选三档与 SQL 不变·topRows 去 imp 预切（预切=按 imp 排序=正被替换的那一项）。
  const pool = [...pinnedRows, ...keywordRows, ...topRows];
  return pool
    .map(m => ({ m, s: scoreMemoryForRecall(m, keywords) }))
    .sort((a, b) => (b.s - a.s) || ((Number(b.m.importance) || 5) - (Number(a.m.importance) || 5)))
    .slice(0, limit)
    .map(x => x.m);
}

// 🔴 批D·件⑥（D2-3 digest 长尾·>3 月零入口修复）：月窗(3 月)之外的"更早重要片段"——
//   imp≥minImportance 或 locked、created_at 早于 N 天、active 且非勿提（复用件① 召回硬过滤）、
//   排除日/周 summary（避与近窗重复；monthly_summary/压缩精华若高 imp 则作为长尾入选）。
//   locked/高 imp 优先。一句化 + 上限由 buildLongTermDigest 施加。
export function getLongTailImportantMemories(companionId, userId, { olderThanDays = 90, minImportance = 8, limit = 3 } = {}) {
  const db = getDb();
  return db.prepare(`
    SELECT id, memory_type, content, importance, locked, created_at
    FROM companion_memories
    WHERE companion_id = ? AND user_id = ?
      AND datetime(created_at) < datetime('now', '-' || ? || ' days')
      AND (importance >= ? OR locked = 1)
      AND memory_type NOT IN ('daily_summary','weekly_summary')
      ${MEM_RECALL_ACTIVE_FILTER}
    ORDER BY (CASE WHEN locked = 1 THEN 1 ELSE 0 END) DESC, importance DESC, created_at DESC
    LIMIT ?
  `).all(companionId, userId, olderThanDays, minImportance, limit);
}
...
// 提醒"今天"的判定走上海日历（自托管 UTC 下避免提醒错日：B2 安全关键点之一）。
...
function migrateProactiveLastSent() {
  ...
  // P1 开场多样化：跨会话"最近 N 次开场由头类型"（JSON array·只存 type 不存内容）。
  addColIfMissing('companions', 'last_hook_types', 'TEXT');
}
...
// P1 开场多样化：跨会话记账"最近 N 次开场由头类型"（只存 type 不存内容）。
// 🔴 必改2：调用方须保证只在【真实 outbound send 成功后】调用一次（drop/失败路径绝不调）——防幽灵记录。
// 🔴 必改1：'other' 不持久化（无避让价值·会挤掉 N 窗）——此处双保险再挡一次。
// 🔴 必改3：last_hook_types 用 JSON array·parse 失败回退 []；**只本函数可写·故意不进 ALLOWED_FIELDS**（dashboard/通用 PATCH 改不了）。
const HOOK_HISTORY_MAX = 4;
export function recordOpeningHook(companionId, hookType) {
  if (!hookType || hookType === 'other') return;
  const db = getDb();
  const row = db.prepare('SELECT last_hook_types FROM companions WHERE id = ?').get(companionId);
  let arr = [];
  try { const p = JSON.parse(row?.last_hook_types || '[]'); if (Array.isArray(p)) arr = p; } catch { arr = []; }
  arr.push(hookType);
  if (arr.length > HOOK_HISTORY_MAX) arr = arr.slice(-HOOK_HISTORY_MAX);
  db.prepare('UPDATE companions SET last_hook_types = ? WHERE id = ?').run(JSON.stringify(arr), companionId);
}
...
// A刀（2026-06-20）：标记本离开周期已尝试 away_probe（epoch 秒·镜像 last_lastcall_at）。
export function markAwayProbeSent(companionId) {
  const now = Math.floor(Date.now() / 1000);
  getDb().prepare(`UPDATE companions SET last_away_probe_at = ? WHERE id = ?`).run(now, companionId);
}
...
export function persistContextToken(botId, userId, token) {
  ...
  `).run(botId, userId, token, clock.now());
}
...
export function loadPersistedContextToken(botId, userId, maxAgeMs = 24 * 60 * 60 * 1000) {
  ...
  `).get(botId, userId);
  if (!row) return null;
  if (clock.now() - row.updated_at > maxAgeMs) return null;
  return row.token;
}
...
// 纪念日（认识 100 天 / 一周年）日期走上海日历，避免自托管 UTC 下纪念日错日（B2）。
function fmtYmd(d) {
...
// D2 贴纸轮次冷却（reply/proactive 共享预算·零时钟依赖）：最近 windowMsgs 条【发出的】消息里
// 若已有贴纸（content 形如 [STICKER:…]）→ 判定冷却中，本轮不注入 hint、出口硬 drop。
// 复用 wechat_messages 现成账·零新表；纯轮次口径（不看时间）→ CI 可确定性断言。
// fail-open：查询异常/缺 id → 返回 false（当作不冷却=放行，节流是质感非安全，不因 DB 抖动崩回复）。
export function recentOutboundStickerActive(wechatUserId, botId, windowMsgs = 4) {
  if (!wechatUserId || !botId) return false;
  try {
    const db = getDb();
    const rows = db.prepare(`
      SELECT content FROM wechat_messages
      WHERE from_user = ? AND to_user = ? AND direction = 'out'
      ORDER BY created_at DESC
      LIMIT ?
    `).all(botId, wechatUserId, Math.max(1, windowMsgs | 0));
    return rows.some(r => typeof r.content === 'string' && /^[\[【]\s*STICKER:/i.test(r.content));
  } catch {
    return false;
  }
}
...
// ─── 上海时钟（时/分/秒/年月日）共享 util ──────────────────────────────────────
// getHours/getMinutes 等本地时间 API 在系统 TZ≠Asia/Shanghai（如自托管 UTC）时偏 8h，
// 导致睡眠窗/早安窗/作息固化 03:40/夜间静默闸等错时。这里统一走 Intl(Asia/Shanghai)。
// 🔴 全部带可选 Date 参（默认 new Date()）、不在内部硬写时钟当唯一源——调用方继续传
// clock.nowDate()/context.now/new Date(ms)，使时区转换与可注入时钟正交解耦（生产零行为变化）。
export function shanghaiHM(date = new Date()) {
...
export const shanghaiHours   = (date = new Date()) => shanghaiHM(date).hour;
export const shanghaiMinutes = (date = new Date()) => shanghaiHM(date).minute;

// 上海日历的 年/月/日（数值）——用于"今天"/纪念日/成年校验等日期级逻辑（避免 UTC 差一天）。
export function shanghaiYMD(date = new Date()) {
  const [year, month, day] = shanghaiDateKey(date).split('-').map(Number);
  return { year, month, day };
}

// 上海时间视觉戳 'YYYYMMDD-HHMMSS'（含秒）——照片时间戳文件名用。
...
// ─── M2: 动态 SET 列名白名单（纵深防御·防 SET 子句注入）──────────────────────
// patchMemory / upsertEmotionState 的列名是裸拼进 SQL 的（值仍参数化）。当前所有
// caller 都只喂内部/已过滤的键，打不穿；但这是"靠 caller 自觉"的设计——加内部白名单，
// 防未来某个 caller 直传 req.body 造成列名注入。**裁定：跳过非法键（不抛·不破调用方）+ 非prod warn**。
const MEMORY_PATCH_FIELDS = new Set([
  'content', 'memory_layer', 'memory_weight', 'memory_source',
  'memory_status', 'importance', 'pinned', 'locked', 'do_not_mention',
]);

// emotion_state 白名单 = schema-derived（PRAGMA·缓存一次），自动纳入未来 ALTER 加的情绪列，
// 排除主键 companion_id 与 updated_at（后者由函数单独追加），保证不破任何写真实列的 caller。
let _emotionStateCols = null;
function emotionStateColumns() {
  if (_emotionStateCols) return _emotionStateCols;
  const rows = getDb().prepare('PRAGMA table_info(companion_emotion_state)').all();
  _emotionStateCols = new Set(
    rows.map(r => r.name).filter(n => n !== 'companion_id' && n !== 'updated_at')
  );
  return _emotionStateCols;
}

// 只放行白名单内、且形如合法标识符的列名；其余跳过（杜绝空格/引号/=/逗号/分号/-- 等注入字符）。
function buildSafeSet(fields, allow) {
  const cols = [], vals = [];
  for (const [k, v] of Object.entries(fields || {})) {
    if (!allow.has(k) || !/^[A-Za-z_][A-Za-z0-9_]*$/.test(k)) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn(`[db] buildSafeSet 跳过非白名单/非法列名: ${JSON.stringify(k)}`);
      }
      continue;
    }
    cols.push(`${k} = ?`);
  ...
  return { sets: cols.join(', '), vals };
}

export function patchMemory(memoryId, companionId, fields) {
  const db  = getDb();
  const now = new Date().toISOString();
  const { sets, vals } = buildSafeSet(fields, MEMORY_PATCH_FIELDS);
  if (!sets) return;  // 无合法列名 → 不执行空 UPDATE（顺手避免无效 SQL）
  db.prepare(`UPDATE companion_memories SET ${sets}, updated_at = ? WHERE id = ? AND companion_id = ?`)
    .run(...vals, now, memoryId, companionId);
}
...
export function upsertEmotionState(companionId, fields) {
  ...
  const { sets, vals } = buildSafeSet(fields, emotionStateColumns());
  if (sets) {  // 空/全非法 fields → 只确保行存在（ensureRow），不发无效空 UPDATE
    db.prepare(`UPDATE companion_emotion_state SET ${sets}, updated_at = ? WHERE companion_id = ?`)
      .run(...vals, now, companionId);
  }
