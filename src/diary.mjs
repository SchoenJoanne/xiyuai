...
import { DIARY_LENGTH_DESC, DIARY_MAX_TOKENS } from './diary_config.mjs';   // A3: 日记长度/tokens 单源
...
5. 长度 ${DIARY_LENGTH_DESC}，不要分点，就是一段自然的日记
...
export async function generateDiaryForCompanion(companionId, opts = {}) {
  ...
  try {
    ...
    const raw = await extractStructuredInfo(
      ...
      prompt,
      { maxTokens: DIARY_MAX_TOKENS, temperature: 0.85, accountId: opts.accountId ?? null },
    );
