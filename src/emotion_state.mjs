...
function computeDelta(userText = '', context = {}) {
  ...
  // Time-of-day energy（睡眠窗 energy 衰减 22:00-07:00 走上海时区）
  // 🔴 B1：本文件无 clock import 且真实消息路径 caller 从不设 context.now，
  // 故用 shanghaiHours(context.now || new Date())——绝不能写 clock.nowDate()（会 ReferenceError）。
  const hour = shanghaiHours(context.now || new Date());
  if (hour >= 22 || hour < 7) {
...
export function updateEmotionFromIdle(companionId, currentState, idleMinutes, affectionLevel = null) {
  ...
  const update = {};
  // 🔴 Step0(2026-06-28) reply reach-guard 独立闸：闸开时不让纯时间推进生成"质问施压"情绪(48h-7天 wronged/cold)。
  // 同 isReplyReachGuardOn(proactive_policy)语义·此处内联避免循环 import。默认关=字节一致旧行为(零变更先验)。
  const _reachOn = /^(1|true|on|yes)$/i.test(process.env.REPLY_REACH_GUARD || '');
  ...
  if (idleMinutes >= 20160) {       // ≥14天 dormant：情绪退潮、放下了——想念回落（不是更惨，是淡了）
    ...
    if (!_reachOn) update.mood = 'cold';           // 🔴 Step0: 闸开不写 idle 驱动的 cold 施压(96h-7天·≥7天退潮另算保留)
  } else if (idleMinutes >= 2880) { // 48-96h disappointed：失望、委屈
    update.dependency = clamp(dep + 8, 0, 100);
    if (!_reachOn) update.mood = 'wronged';         // 🔴 Step0: 闸开不写 idle 驱动的 wronged 施压(想念 dep 仍保留)
...
// 当前上海时间的分钟数（0-1439）——复用共享 shanghaiHM（删本地重复 Intl 实现防漂移）。
function _nowShanghaiMinute(date = new Date()) {
