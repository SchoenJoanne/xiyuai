...
import * as clock from './clock.mjs';   // 24h 主动窗口判定=业务时间·走可注入时钟（生产=真实时间·测试可拨验窗口关/重开）
...
export function rememberContextToken(botId, userId, token) {
  ...
  lastContextTokenByPair.set(ctxPairKey(botId, userId), { token, at: clock.now() });
...
export function recallContextToken(botId, userId, maxAgeMs = 24 * 60 * 60 * 1000) {
  ...
  if (entry && (clock.now() - entry.at) <= maxAgeMs) return entry.token;
  // miss → 回查持久化表（解决进程重启 / 独立脚本场景）
  try {
    const persisted = loadPersistedContextToken(botId, userId, maxAgeMs);
    if (persisted) {
      // 回填内存 cache，下次走快路径
      lastContextTokenByPair.set(ctxPairKey(botId, userId), { token: persisted, at: clock.now() });
      return persisted;
    }
  } catch { /* DB miss 不阻塞 */ }
  return null;
...
async function getTypingTicket(ctx, ilinkUserId, contextToken) {
  ...
  try {
    ...
    if (ticket) typingTicketCache.set(key, { ticket, at: Date.now() });   // typing ticket TTL=实现细节·不走 clock(与 L558 读端一致)
    return ticket;
  } catch {
    return null;
  }
