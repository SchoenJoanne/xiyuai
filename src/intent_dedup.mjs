 ...
 *
 * 背景（PR-0 retention_board 实测）：4 例"第二天回来"的存量对话里**全部**检出 hard break，
 * 复读为主——最该留住的人最容易撞复读。现有 anti-repeat（proactive 3-gram Jaccard）只拦**文本**，
 * 挡不住"语义功能相同、文本不同"的复读（某存量案例 一晚 remind_安全 + ask_plan 出 3-4 次）。
 ...
 */
export function isIntentCooled({ intent, topic = '', events = [], nowMs = clock.now(), ackStreak = 0 }) {
...
export function buildIntentDedupHint(events, nowMs = clock.now()) {
