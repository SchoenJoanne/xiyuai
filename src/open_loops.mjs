...
import { expireDaysForKind } from './open_loops_lifecycle.mjs';   // 批D·件⑤：past 封顶天数单源（零依赖叶子）
 ...
 */
// ─── 带时效事件的确定性时间状态（停板B·time-perception fix）───────────────────────
// 🔴 确定性代码算好状态·不让 LLM 算日期(同危机档"关键的事不进 LLM")。上海零点 date-only 作差·杜绝时区漂移。
//   参照 reminders.isReminderDue(db.mjs:4424 每天拿 today 实比) / examCountdownFor(life_calendar.mjs:67-80)。
//   返回 null=无时效(due_at 空/非法/脏·fail-open 退回现状不加标注)；否则 { phase, tag }。
//   phase: future(≥2天) / tomorrow(1) / today(0) / past(≤-1)。tag=给 LLM 的确定性状态短语(过期带行动指引)。
export function dueStatus(dueAt, todayKey, loopKind = null) {
  if (!dueAt || !/^\d{4}-\d{2}-\d{2}$/.test(String(dueAt))) return null;        // 无 due_at / 非法格式 → 无时效
  if (!todayKey || !/^\d{4}-\d{2}-\d{2}$/.test(String(todayKey))) return null;  // today 脏 → fail-open
  const d = Date.parse(dueAt + 'T00:00:00+08:00');
  const t = Date.parse(todayKey + 'T00:00:00+08:00');
  if (Number.isNaN(d) || Number.isNaN(t)) return null;                          // fail-open：退回现状·不崩不误标
  const days = Math.round((d - t) / 86400000);
  if (days >= 2)  return { phase: 'future',   tag: `〔还有${days}天〕` };
  if (days === 1) return { phase: 'tomorrow', tag: '〔就在明天〕' };
  if (days === 0) return { phase: 'today',    tag: '〔就是今天·可以关心一下〕' };
  // 🔴 批D·件⑤：past 封顶——过期超其 kind 的 N 天 → 'expired'（不再"越过期越催"·⑥ 由 buildOpenLoopsHint 滤掉不注入）。
  //   三处同源之一（叶子 expireDaysForKind）。补夜批 markStale 未跑窗口（读时现算·不依赖 cron 活着）。
  const overdue = -days;
  if (overdue > expireDaysForKind(loopKind)) return { phase: 'expired', tag: '' };
  return { phase: 'past', tag: `〔已经过去${overdue}天了·该问问后来了，别当成还没发生〕` };
}
...
export function buildOpenLoopsHint(companionId) {
  try {
    ...
    const today = shanghaiDateKey(new Date());   // 确定性今天(上海)·喂 dueStatus 算真实时间状态
    // 🔴 批D·件⑤ ⑥：读侧封顶——过期超 N 天(dueStatus phase='expired')的 loop 直接不注入（"翻旧账"止·
    //   补夜批 markStale 未跑窗口）。每条带 loop_kind 判过期。
    const active = loops
      .map(l => ({ l, st: dueStatus(l.due_at, today, l.loop_kind) }))
      .filter(x => !(x.st && x.st.phase === 'expired'));
    if (!active.length) return '';
    // appointment（你俩的约定）置顶，其余保持 listOpenLoops 既有排序(emotional_weight/due_at)
    const sorted = active.sort((a, b) =>
      (b.l.loop_kind === 'appointment' ? 1 : 0) - (a.l.loop_kind === 'appointment' ? 1 : 0));
    const lines = sorted.slice(0, 4).map(({ l, st }) => {
      // 🔴 appointment "还没兑现"后缀只在未过期时加；已过期由〔已过去…〕统一表达(避免"还没兑现〕〔已过去〕"冗余矛盾)。
      const appt = (l.loop_kind === 'appointment' && !(st && st.phase === 'past')) ? '（你俩的约定，还没兑现）' : '';
      return `- ${String(l.title).slice(0, 60)}${appt}${st ? ` ${st.tag}` : ''}`;
    });
    // 🔴 注入头改写：以〔〕状态为准(别被描述里"明天/后天"误导)；未来可提起、已过去问后来/收尾不复读。
    return `\n【你还记得这些没了结的事】（每条后面〔〕里是它到今天的真实时间状态，以此为准，别被描述里的"明天/后天"误导——还没到的可以自然关心或提起；已经过去的要问问后来、或收个尾，别再当成还没发生。）\n${lines.join('\n')}`;
  } catch {
    return '';
  }
}
 ...
 * 用启发式（关键词 + 时间距离），不调 LLM 控成本。命中后调 resolveOpenLoop()。
 *
 * 🔴 批D·件⑤（考试案四环·Ring①）：此启发式是**尽力关闭**（与门=RESOLVE_KEYWORD + title 核心词·
 *   刻意保守防误关）。漏关的 loop 不会永远 open——由**过期 backstop 兜底**：markStaleOpenLoops 夜批
 *   按 kind 分层落 'expired'（时效类 2 天/现役 7 天/无 due 14 天）+ dueStatus 读侧封顶 + follow_up_count
 *   注入上限=3。四环合力=「关不上也会过期消失」，故此处宁保守勿误关（误关=丢真未了结事·更伤）。
 */
