...
import { getDb, VOICE_STYLE_ENUM, clampVoiceSpeed, PERSONA_PROMPT_MAXLEN } from './db.mjs';   // v1.22 voice security:同源枚举/clamp · persona_prompt 硬化:同源长度上限
...
const IMPORT_STRING_FIELDS = new Set([
  ...
  'chat_mode_active', 'proactive_frequency', 'proactive_time_window',
  'attachment_style', 'locale',
]);   // v1.22 voice security:voice_style 移出自由文本·改入 IMPORT_ENUM_VALUES(枚举验证)
...
const IMPORT_ENUM_VALUES = new Map([
  ...
  ['voice_style', VOICE_STYLE_ENUM],   // v1.22 voice security:导入侧枚举验证·非法值落 DEFAULTS(温柔)·防注入入库
]);
...
export function sanitizeImportedCompanion(payload) {
  ...
  for (const [key, val] of Object.entries(raw)) {
    if (IMPORT_ENUM_VALUES.has(key)) {
      ...
      // v1.22 persona_prompt 硬化:导入侧同源长度上限(其余 string 字段维持 4000)
      const s = String(val ?? '').slice(0, key === 'persona_prompt' ? PERSONA_PROMPT_MAXLEN : 4000);
      ...
      companionFields[key] = Number.isFinite(f)
        ? (key === 'voice_speed' ? clampVoiceSpeed(f) : f)   // v1.22 voice security:voice_speed 夹 [0.5,2.0]·temperature/top_p 不动
        : undefined;
...
export async function importCompanionForUser(userId, accountId, botId, payload, options = {}) {
  ...
  // 🔴 合规确定性底（《暂行办法》·路线A）：导入 age 夹到最低 18（堵导入后门·不丢导入·age:15→18）。
  //    persona INSERT 是独立写路径（不过 createCompanion 咽喉），故在此 db 写咽喉自带 floor = 与 createCompanion 拒<18 同源。
  row.age = Math.max(18, Number(row.age) || 20);
