...
// Verbs that, following the companion's OWN name, mark self-reference in the third
// person ("<name>觉得…" where a real person would say "我觉得…"). Shared by the detector
// (checkPersonaConsistency) and the repair (sanitizeReplyByGuard) so the two never drift.
// 🔴 The name is bound per-companion from companion.name at call time — never hardcode a
// specific name here (see bot.mjs "角色名用 companion.name 动态插值·勿硬编溪语").
const SELF_REF_VERBS = ['觉得', '认为', '想', '以为', '感到', '希望', '会', '要', '想要', '打算'];

// Escape regex metacharacters so a companion name like "A.B" or "小(满)" is matched
// literally rather than as a pattern.
function escapeRegExp(s) {
  return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// Build the self-third-person detector for a specific companion name.
// Returns null when there is no usable name (nothing to anchor against → guard no-ops, fail-open).
function buildSelfThirdPersonRe(name) {
  if (!name || !name.trim()) return null;
  return new RegExp(`${escapeRegExp(name)}(?:${SELF_REF_VERBS.join('|')})`);
}
...
export function checkPersonaConsistency(reply, context = {}) {
  ...
  const selfThirdRe = buildSelfThirdPersonRe(companion.name);
  if (selfThirdRe && selfThirdRe.test(reply)) {
    reasons.push('self_third_person');
  }
...
export function sanitizeReplyByGuard(reply, context = {}) {
  ...
  // Fix self-third-person: "<name>觉得" → "我觉得". Name built dynamically from
  // companion.name, escaped, verbs shared with the detector (SELF_REF_VERBS).
  if (companion.name && companion.name.trim()) {
    const nameRe = new RegExp(`${escapeRegExp(companion.name)}(?=${SELF_REF_VERBS.join('|')})`, 'g');
