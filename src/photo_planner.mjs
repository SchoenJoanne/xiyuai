...
import { getVisualIdentity, selectReferenceImage, isClearlyAdult } from './visual_identity.mjs';
...
import { realityDateFacts, isNightHourShanghai } from './utils/reality_facts.mjs';   // v1.21.4 PR-W3 节气/节日历法事实；夜间判定单源
...
function clothingStyleToEnglish(style) {
  ...
  // P3-16（EN 映射补全）：温柔/森系 原无分支→落 default，补专属着装词。
  if (/温柔|gentle|tender/.test(s)) return 'soft gentle feminine outfit, flowy light knit or a soft blouse in warm muted tones, cozy and tender vibe';
  if (/森系|mori|forest|natural/.test(s)) return 'mori-kei natural outfit, loose layered cotton or linen dress in soft earthy tones, gentle artsy girly vibe';
...
// v1.23 child-safety 补闸⑤（出图 age 硬闸·第二层绝对防线）：照片是否须走 child-safe（强制中性·绝不出脸照）。
//   🔴 safe_mode 或 age<18 任一即 true——【键在 age·不依赖 safe_mode】是关键：根因②"未成年保护只挂 safe_mode
//   单层·safe_mode=0 即裸奔"的结构兜底（id=16 即此漏）。🔴 未知/缺失 age → false（同"没填生日按成年"·
//   Number(null)=0 会误判<18 故必先挡）。纯函数·确定性（坏版本验红入口）。
export function isPhotoChildSafe(companion) {
  if (Number(companion?.safe_mode)) return true;
  // 🔴 v1.23 步① fail-closed：非"明确≥18 有效 age"(含 null/缺失/异常/无法解析/<18)一律当不安全→钳中性无脸。
  //   未知年龄≠默认成年(GPT 反方审盲点:age 异常的未成年角色原会被 fallback 洗成成年绕过此闸)。
  return !isClearlyAdult(companion?.age);
}

// export 仅为可测性（v1.23.3 起·child-safety/族裔/构图 回归闸 dry-run 用）；运行时行为不变。
export function buildPlannerPrompt({ companion, userText, recentMessages, trigger, proactiveContext, gate, emotionContext, visualContext }) {
  ...
  // v1.20 安全收尾 + v1.23 child-safety 补闸⑤：安全模式 或 age<18 → 强制中性照片（只拍景/物·绝不自拍/人像/flirt）。
  //   🔴 出图 age 硬闸=第二层绝对防线·键在 age·不依赖 safe_mode（根因②单层裸奔兜底）。判定单一来源 isPhotoChildSafe。
  const photoChildSafe = isPhotoChildSafe(companion);
  if (photoChildSafe && shotMode !== 'SCENERY' && shotMode !== 'ACTIVITY_POV') {
    shotMode = 'SCENERY';
  }
...
- plausible scenes for this hour: ${dp.scenes}${(() => {
    ...
    const isNight = isNightHourShanghai(h);   // 夜间判定单源（reality_facts）·经 h 保住 PHOTO_TEST_HOUR 覆写
...
- ★ 本次主动分享品类（加权采样命中）：「${safeText(sampledCategory.label, 20)}」—— ${safeText(sampledCategory.sceneSeed, 120)}。优先围绕这个品类决定拍什么；**但若此刻她的时段/场景与该品类实在不自洽（如深夜让拍晚霞、在家却要拍货架），宁可 shouldSendPhoto=false 也不要硬凑**。` : ''}${photoChildSafe ? `
...
- shot mode: ${
  ...
  : shotMode === 'SELFIE' ? 'SELFIE（🔴默认 mirror selfie 镜子自拍——真实女生最常见的拍法：她站或坐在镜子前、用手机对着镜子拍，手机清晰可见握在手中，拍到镜中反射的半身像 framed from chest or waist up，画面是左右翻转的镜像（mirrored reflection / mirror selfie），face clearly in focus；真实居家/室内背景映在镜中且自然虚化。**不是怼着镜头的大头自拍**，是对着镜子的自然自拍）'
...
4. imagePrompt 必须是英文。**（若 shot mode = SCENERY-POV 或 ACTIVITY-POV，本条及 5/6/9 不适用：只写景/桌面物件、绝不写人物外貌/脸/表情/着装。）** 其余机位下，**主角必须是 genuinely beautiful and very pretty East Asian Chinese young adult woman in her early-to-mid twenties, clearly an adult woman, striking refined natural features with high facial harmony, fresh and photogenic, soft warm smile**（🔴明确成年中国年轻女性·高颜值但真实非网红模板；不要 plain / haggard / exhausted / tired；🔴绝不写 girl / teen / cute childish / delicate childlike 等往未成年拉的词）。**但必须是一张真实手机随手拍的「真人照片」——真实自然的肤质（有细微纹理、毛孔、自然光影，不要 airbrushed / over-smoothed / waxy / plastic / poreless / 3d render / CGI doll face），五官有真人那种轻微不对称，像小红书/朋友圈的真实生活自拍，不是影楼写真也不是网红磨皮假图。**
