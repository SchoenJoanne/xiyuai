...
import {
  ...
  shanghaiHM,
  getRecentSummaries, getLongTailImportantMemories,   // v1.28 批D·件⑥ digest 长尾（>3 月零入口修复）
...
import { identityRoutineHint, routineSleepBaseline } from './routine_profiles.mjs';   // 五身份作息档（替代 ageOccupationHint）+ 作息基线（wake-align 回退）
import { getDayContext } from './life_calendar.mjs';            // 学期/寒暑假/法定节假日（单一事实源·同喂 anchor）
...
import { refreshSocialCircle } from './social_circle.mjs';   // 社交圈 Step1 档案推进（灰度闸默认关·age<18 排除）
  ...
  for (const row of rows) {
    try {
      ...
      if (fresh.is_sleeping && fresh.today_wake_at && nowMs >= fresh.today_wake_at + 5 * 60_000) {
        ...
        // missed 解耦(i)（v1.23·SLEEP_GUARD）：fallback exitSleep（urgent morning 被节流/失败/disabled）
        // 也排空 missed——否则睡眠期攒的消息永久躺 consumed=0（本次 P0 真凶之一）。无早安可承接故静默 drain
        // （safety net）；用户醒后开口的承接走 bot.mjs reply 路径(ii)。闸关=不 drain（旧行为·零变更）。
        if (isSleepGuardOn()) {
          try { const _d = drainMissed(row.companion_id); if (_d.length) log('info', `[Sleep] fallback 排空 missed=${_d.length} companion=${row.companion_id}`); }
          catch (e) { log('warn', `[Sleep] fallback drainMissed failed companion=${row.companion_id}: ${e.message}`); }
        }
        log('info', `[Sleep] fallback exitSleep companion=${row.companion_id} (no morning sent within 5min of wake)`);
      }
      // 2) 学习固化：每天 03:40 cron 时尝试（避开高峰）——走上海时区，避免自托管 UTC 下偏移到 11:40
      const _hm = shanghaiHM(now);
      if (_hm.hour === 3 && _hm.minute === 40) {
...
async function runDailySchedules(dateKey, weekdayLabel, weekdayNum) {
  const db = getDb();
  const companions = db.prepare(`
    SELECT c.id, c.user_id, c.name, c.age, c.role_title, c.personality_tags, c.hobbies, c.safe_mode, c.life_identity, c.life_subform
    FROM companions c
    JOIN users u ON u.id = c.user_id
    JOIN wechat_accounts wa ON wa.wechat_user_id = u.wechat_user_id
    WHERE wa.is_active = 1
  ...
  for (const comp of companions) {
    ...
    // 社交圈 Step1：搭同一日程批便车推进社交档案（🔴灰度闸默认关 + age<18 排除 冻结存量未成年·内部 fail-open·
    // 绝不阻断日程）。Step1 只写 B 表 companion_social_circle·零 proactive social·零 memories 写入。
    try { await refreshSocialCircle(comp); } catch (e) { log('warn', `[SocialCircle] 推进失败 companion=${comp.id}: ${e.message}`); }
    try {
...
// ─── v1.23 作息双源对齐（SCHEDULE_WAKE_ALIGN·默认关=零变更先验）──────────────────
// 让 daily 日程的"起床项"贴 sleep_schedule 权威（行为硬判定源·唯一消费用户手设+学习偏移）。
// 🔴 单向 sleep→daily：只读 sleep 表喂提示词·绝不写/绝不碰判睡逻辑（maybeSleepBlock/isSleepingNow/wake±45/away_probe 全不动）。
// 🔴 软约束不钉死毫秒（她仍可自然赖床）。🔴 冻结存量未成年（age<18）明确排除·不进对齐（保持现状双源·同 min-age/今天所有改动排除 冻结存量未成年）。
export function isScheduleWakeAlignOn() { return /^(1|true|on|yes)$/i.test(process.env.SCHEDULE_WAKE_ALIGN || ''); }

// 🔴 只读取"她今天大约几点醒"（分钟·上海）：优先当天已刷新的 sleep today_wake_at（直接 SELECT·绝不调会写的 getOrRefreshTodaySchedule）·
//    null/stale 回退身份基线 routineSleepBaseline.wakeMin（identity-appropriate·🔴绝不回退硬编码 07:30）。
export function resolveAlignWakeMin(comp, dateKey, dayContext) {
  try {
    const row = getDb().prepare('SELECT today_wake_at, today_date FROM companion_sleep_schedule WHERE companion_id = ?').get(comp.id);
    if (row && row.today_wake_at && row.today_date === dateKey) {
      const d = new Date(Number(row.today_wake_at));
      return ((d.getUTCHours() + 8) % 24) * 60 + d.getUTCMinutes();   // 上海时区分钟
    }
  } catch { /* 读不到走身份基线回退 */ }
  try {
    const b = routineSleepBaseline(comp, (dayContext && dayContext.dayKind) || 'active');
    return (b && Number.isFinite(b.wakeMin)) ? (b.wakeMin % 1440) : null;
  } catch { return null; }
}

// 软约束 hint（gated）：闸关 或 冻结存量未成年（age<18）→ 返 ''（零变更 / 明确跳过）。喂进日程生成提示词·不钉死毫秒。
export function buildWakeAlignHint(comp, dateKey, dayContext) {
  if (!isScheduleWakeAlignOn()) return '';
  if (Number(comp.age) < 18) return '';   // 🔴 冻结存量未成年 明确不进对齐（保持现状双源·绝不改其运行时表现）
  const wakeMin = resolveAlignWakeMin(comp, dateKey, dayContext);
  if (wakeMin == null) return '';
  const hh = String(Math.floor(wakeMin / 60) % 24).padStart(2, '0');
  const mm = String(wakeMin % 60).padStart(2, '0');
  return `\n🔴 她平时大约 ${hh}:${mm} 起床（这是她真实作息）——今天日程的起床/醒来项要贴近这个点，可自然赖床十几二十分钟，但别写成晚一两小时才醒（别动不动"九点十点才自然醒"）。用自然口吻（"七点多自然醒""赖床到八点"），别精确报分钟。`;
}

// v1.23 日程 P1（去双锚·职责切开）：作品进度的【单一权威源=works 档案】。continuity（昨日生活线）
// 注入前过滤掉"与 active works 标题重名"的昨日条目——让 works 独家负责"作品类"延续（追剧/看书/学技能的
// 进度由 progress_note 说了算），continuity 只承接"非作品类"生活事件（去图书馆/和朋友吃饭/帮室友改简历…）。
// 🔴 纯函数·无 IO·确定性（坏版本验红入口）。works 空 / 无重名 → 原样返回（零误伤·正常 companion 行为不变）。
//    标题取核心串（去《》去空白·长度≥2 防 1 字标题过度匹配）；昨日条目含任一核心标题即判为作品类重名。
// 🔴 冻结存量未成年(age<18)：直接原样返回·不过滤（运行时 byte-identical·与 wakeAlign 同口径排除存量未成年）。
const _stripWorkBrackets = (s) => String(s || '').replace(/[《》]/g, '').trim();
export function filterContinuityThreadsByWorks(threads, works, { age = 99 } = {}) {
  const list = Array.isArray(threads) ? threads.filter(Boolean) : [];
  if (Number(age) < 18) return { kept: list, dropped: [], skipped: 'minor' };   // 🔴 冻结存量未成年 冻结·不过滤
  if (!list.length) return { kept: [], dropped: [] };
  const titles = (Array.isArray(works) ? works : [])
    .map((w) => _stripWorkBrackets(w && w.title))
    .filter((t) => t && t.length >= 2);
  if (!titles.length) return { kept: list, dropped: [] };   // 无 works 标题 → 不过滤（零误伤）
  const kept = [], dropped = [];
  for (const t of list) {
    (titles.some((title) => String(t).includes(title)) ? dropped : kept).push(t);
  }
  return { kept, dropped };
}
...
async function generateScheduleFor(comp, dateKey, weekdayLabel, weekdayNum) {
  ...
  // 学期/寒暑假/法定节假日/星期几（单一事实源·同喂 buildPresenceAnchor）
  const dayContext = getDayContext(comp, dateKey);
  // 16-35 走五身份作息档（含子形态+个体偏移）；35+ 维持现状 age 档兜底（用户裁定⑥）
  const occupationHint = (age > 35 && !comp.life_identity)
    ? ageOccupationHint(age, isWeekend, comp.role_title)
    : identityRoutineHint(comp, dayContext);
  // v1.23 作息双源对齐：起床项贴 sleep 权威（gated SCHEDULE_WAKE_ALIGN·默认关零变更·🔴冻结存量未成年 排除·只读 sleep 不写）
  const wakeAlignHint = buildWakeAlignHint(comp, dateKey, dayContext);

  // v1.12.0「她有自己的连续生活线」：把昨天日程里值得延续的事喂进来，让今天不是全新的人和事。
  // v1.23 P1（去双锚）：注入前过滤掉与 active works 重名的"作品类"条目——作品进度【单一权威源=works】，
  //   continuity 只承接非作品生活事件，消除"continuity 报昨日旧集数 / works 报档案新集数"的双锚打架。
  //   works 取数自带 fail-open（取不到→不过滤=原行为·与下方 worksScheduleHint 各自独立·不动其原路径）；
  //   🔴 冻结存量未成年(age<18) 在 filter 内部直接放行不过滤·byte-identical。
  let continuityHint = '';
  try {
    const y = getDailySchedule(comp.id, addDays(dateKey, -1));
    let threads = (y?.items || [])
      .filter(it => (it.importance || 0) >= 5)
      .map(it => it.activity).filter(Boolean);
    let worksForDedupe = [];
    try { worksForDedupe = getActiveCurrentWorks(comp.id) || []; } catch { /* 取不到 works → 不过滤·continuity 原行为 */ }
    threads = filterContinuityThreadsByWorks(threads, worksForDedupe, { age: comp.age }).kept;
...
今天日期：${dateKey} ${weekdayLabel}（${dayContext.label}）

【强制约束 - 极其重要】
${occupationHint}${wakeAlignHint}
${continuityHint}
${worksScheduleHint}
...
- 要符合「${dayContext.label}」的真实节奏（${dayContext.dayKind === 'break' ? '假期作息和平时完全不同，别按上学/上班写' : dayContext.dayKind === 'rest' ? '周末/休息日节奏' : '工作日/上学日节奏'}）
- 不同身份不同活动：学生别去酒吧/别上班，上班族别上课，备考党在自习室
...
严格输出 JSON：
{
  ...
  log('info', `[DailySchedule] companion=${comp.id} ${comp.name} ✓ ${items.length} 段 ${dayContext.label} segments=${moodSegments ? '3' : '0'}`);
}
...
export async function ensureScheduleForCompanion(companionId, dateKey) {
  ...
  const db = getDb();
  const comp = db.prepare(`SELECT id, name, age, role_title, personality_tags, hobbies, life_identity, life_subform FROM companions WHERE id = ?`).get(companionId);
  ...
  for (const [groupMonth, groupRows] of groups.entries()) {
    ...
    try {
      ...
      saveMemory({
        ...
        memoryType: 'monthly_summary',
        content: `${prefix}${summary}`,
        importance: 10,  // 压缩精华·优先召回——钳在合法域 CHECK(1-10) 上限（chip③：原写 15 撞 CHECK→月压缩批必败→原条目永不压缩）
      });
 ...
 * 素材」的一条**旁路**。生产实锤 daily_summary 高价值形态经此路反复复读 4 次/天：素材级去重
...
export async function buildLongTermDigest(companionId, userId, { excludeUsedIds = null } = {}) {
  ...
  // 🔴 批D·件⑥（D2-3）：>3 月零入口修复——月窗(3 月)之外的"更早重要片段"(imp≥8||locked·一句化·上限 3)。
  //   同过素材冷却闸门(cool·proactive 剔除近期已用·reply 全保留)=延续「注入动作挂闸门」铁律(见函数头 ⚠⚠)。
  const longTailGist = (s) => {
    const t = String(s || '').trim();
    const head = (t.match(/^[^。！？!?\n]*[。！？!?]?/) || [t])[0].trim();
    return head.length > 40 ? head.slice(0, 40) + '…' : head;
  };
  const longTail = cool(getLongTailImportantMemories(companionId, userId, { olderThanDays: 90, minImportance: 8, limit: 3 }));
  if (longTail.length > 0) {
    blocks.push('【更早的重要片段（很久以前，但值得记得）】\n' + longTail.map(m => `- ${longTailGist(m.content)}`).join('\n'));
  }
