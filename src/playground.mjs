...
export async function playgroundChat(companion, userText, { probe = false } = {}) {
  ...
  // 贴纸节流 v1.27：预览对齐生产——D1 整条至多 1 张（跨段递减预算）+ enabled（开关关→出口硬 drop）；
  // 无 D2 冷却（预览无真实发送时间线）。
  let stickerBudget = stickerEnabled ? 1 : 0;
  const richSegments = segments.map(seg => {
    const { text: t, stickers } = parseStickerMarkers(seg, { max: stickerBudget, enabled: stickerEnabled });
