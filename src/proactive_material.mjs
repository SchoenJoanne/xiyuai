...
// ── ③ 环境fact去重（v1.26）：月相等"真实世界事实"按【相位档】冷却，专治 proactive 每晚重复"今晚月亮特别圆" ──
/** 环境事实冷却天数——独立于素材 14 天。相位窗≈3.7 天，默认 5 覆盖窗+±1 计算抖动、远短于朔望月 29.5→不跨月永冷（env PROACTIVE_ENV_FACT_DEDUP_DAYS 覆盖） */
export const DEFAULT_ENV_FACT_DEDUP_DAYS = 5;
export function envFactDedupDays(env = process.env) {
  const n = Number(env.PROACTIVE_ENV_FACT_DEDUP_DAYS);
  return Number.isFinite(n) && n >= 0 ? n : DEFAULT_ENV_FACT_DEDUP_DAYS;
}
/** 环境事实素材 ID 规范：月相 env:moon:<phaseName>（🔴 无日期后缀——同一相位窗连续几晚同 id 才能被冷却抑制；加日期会让 id 每晚变→去重失效） */
export function envMoonMaterialId(phaseName) { return `env:moon:${phaseName}`; }
/**
 * ③ 月相注入 gate（纯逻辑·无 IO·可单测）。仅 proactive 口调用；reply/photo 一律不调（物理隔离）。
 * @param {{isNight:boolean, phaseName:string, festival:string, usedIds:Set<string>}} o
 * @returns {{includeMoon:boolean, moonToLog:(string|null)}}
 *   includeMoon=false → 冷却内·本晚不注入月相（节气/节日/其它 reality 字段照常）
 *   moonToLog!=null   → 将注入（含节日豁免）→调用方注入后落账
 * 🔴 中秋/元宵豁免：只跳"读冷却"、moonToLog 仍返回→当晚必有月亮且照常落账（次晚同相位正常去重不刷屏）。
 * 🔴 fail-open 由调用方 try/catch 兜（gate 本身纯函数不抛；usedIds 非 Set 视作无冷却=不抑制）。
 */
export function resolveEnvMoonInclusion({ isNight, phaseName, festival, usedIds } = {}) {
  if (!isNight || !phaseName) return { includeMoon: false, moonToLog: null };
  const moonId = envMoonMaterialId(phaseName);
  const moonFestival = festival === '中秋' || festival === '元宵';   // 应景豁免（只跳读冷却）
  if (!moonFestival && usedIds instanceof Set && usedIds.has(moonId)) {
    return { includeMoon: false, moonToLog: null };                  // 冷却内→本晚不提月亮
  }
  return { includeMoon: true, moonToLog: moonId };                    // 注入（含节日）→待落账
}
