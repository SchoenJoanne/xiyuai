...
async function azureSynthesize({ region, apiKey, voice_id, speed, text, signal }) {
  ...
  // v1.22 voice security 纵深：azure 自身夹 [0.5,2.0]→rate ∈ [-50%,+100%]·防越界值生成非法 SSML（其余 provider 已各自夹）
  const _sp = Math.max(0.5, Math.min(2.0, Number(speed) || 1.0));
  const ratePct = `${Math.round((_sp - 1.0) * 100)}%`;
