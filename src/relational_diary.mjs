...
import { RELATIONAL_DIARY_LENGTH_DESC, RELATIONAL_DIARY_MAX_TOKENS } from './diary_config.mjs';   // A3: 日记长度/tokens 单源
...
请写一段「今天和他之间的回忆片段」——${RELATIONAL_DIARY_LENGTH_DESC}，以你的视角，聚焦你们之间发生的事，而不是写你自己的心境。要点：
...
export async function generateRelationalDiaryForCompanion(companionId, opts = {}) {
  ...
  try {
    ...
    const raw = await extractStructuredInfo(
      ...
      prompt,
      { maxTokens: RELATIONAL_DIARY_MAX_TOKENS, temperature: 0.85, accountId: opts.accountId ?? null },
    );
    let parsed;
    try {
