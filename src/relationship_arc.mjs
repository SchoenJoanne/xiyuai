...
import { emotionEngineOn } from './emotion_engine.mjs';   // 批E·E3：闸控双路径分派谓词（零依赖叶子·arc→leaf 无环）
...
/**
 * 时间驱动 arc tick 分派入口（批E·E3 Phase3·闸控双路径·E0c 单一谓词 fork 点之一）。
 * 🔴 闸 OFF（默认/无 companionId）→ tickArcOnTimeLegacy（旧行为字节一致）；闸 ON → tickArcOnTimeV2（无 neglect 阶梯）。
 */
export function tickArcOnTime(ctx = {}) {
  if (!emotionEngineOn(ctx.companionId)) return tickArcOnTimeLegacy(ctx);
  return tickArcOnTimeV2(ctx);
}

/**
 * 时间驱动 tick·V2（批E·E3 §2·闸 ON 臂·neglect 时间阶梯全退役）。纯函数。
 * 🔴 缺席不再造 hurt/cold/withdrawing/scar——她的克制不是阉割是健康（长缺席=OU 回基线平静+T3/T4 温暖重逢）。
 * 🔴 本函数体【零缺席量读取】：不碰 neglectStage / NEGLECT_IDX（静态锁·防手滑回渗）；签名保留 neglectStage 入参
 *   （调用方 ctx 不变·V2 忽略之·T3/T4 重逢文案仍在别处用它）。
 * 保留：ops_clamp 保险丝 / hurt faded 互动消化（interactionsSinceEvent 驱动=事件性）/ safe_mode 封顶（_capState）。
 * cold→normal 走 engine_calmed 收敛（runtime 层·Phase4）或道歉事件·非时间；withdrawing sunset 后不可达（ops_clamp 兜底残留）。
 * @param {object} ctx 同 Legacy
 * @returns 同 tickArcOnSignal
 */
export function tickArcOnTimeV2(ctx = {}) {
  const {
    state = 'normal', stateChangedAt = null, safeMode = false, openEvent = null,
    interactionsSinceEvent = 0, now = new Date(),
  } = ctx;
  const res = _mkRes(state);
  const maxState = 'maxState' in ctx ? ctx.maxState : _maxStateFromEnv();
  const go = (next, reason) => {
  ...
  // 钳位保险丝（保留·对任意存量态生效·含 sunset 后残留 withdrawing/scar 未知态）
  if (maxState && (ARC_STATE_RANK[state] ?? 0) > (ARC_STATE_RANK[maxState] ?? 9)) {
    return go(maxState, 'ops_clamp');
  }

  // hurt：faded 互动消化（interactionsSinceEvent 驱动=事件性·保留）——时间阶梯退役
  if (state === 'hurt') {
  ...
  // cold / withdrawing / repairing / normal(_with_scar)：时间阶梯全退役——缺席不再升级或造疤。
  return stay('noop');
}
 ...
 * @returns 同 tickArcOnSignal
 */
export function tickArcOnTimeLegacy(ctx = {}) {
  const {
    state = 'normal', stateChangedAt = null, safeMode = false, openEvent = null,
